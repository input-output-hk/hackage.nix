    ...
    in {
      ...
      components = {
        "mintty" = {
          depends  = [
            hsPkgs.base
          ] ++ pkgs.lib.optionals system.isWindows (if _flags.win32-2-5-3
            then [ hsPkgs.Win32 ]
            else [
              hsPkgs.filepath
              hsPkgs.Win32
            ]);
          build-tools = pkgs.lib.optionals system.isWindows (pkgs.lib.optional (!_flags.win32-2-5-3) hsPkgs.buildPackages.hsc2hs);
        };
      };
    }