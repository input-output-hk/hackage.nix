      ...
      components = {
        "pandoc-citeproc" = {
          depends  = (((((([
            ...
            hsPkgs.split
            hsPkgs.yaml
          ] ++ pkgs.lib.optional (!(compiler.isGhc && compiler.version.ge "8.0")) hsPkgs.semigroups) ++ pkgs.lib.optional (compiler.isGhc && compiler.version.lt "7.6") hsPkgs.ghc-prim) ++ pkgs.lib.optional _flags.debug hsPkgs.pretty-show) ++ pkgs.lib.optional _flags.bibutils hsPkgs.hs-bibutils) ++ pkgs.lib.optional _flags.embed_data_files hsPkgs.file-embed) ++ (if _flags.unicode_collation
            then [
              hsPkgs.text
              hsPkgs.text-icu
            ]
