{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "ghc-exactprint"; version = "1.8.0.0"; };
    ...
    components = {
      "library" = {
        ...
        buildable = if compiler.isGhc && (compiler.version).lt "9.8"
          then false
          else true;
        };
      exes = {
        "roundtrip" = {
          depends = (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "9.8" && flags.roundtrip) [
          ...
          buildable = if compiler.isGhc && (compiler.version).ge "9.8" && flags.roundtrip
            then true
            else false;
          };
        "static" = {
