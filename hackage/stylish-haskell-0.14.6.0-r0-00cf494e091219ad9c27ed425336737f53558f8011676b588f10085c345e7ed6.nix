{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "2.4";
      identifier = { name = "stylish-haskell"; version = "0.14.6.0"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.0") (hsPkgs."semigroups" or (errorHandler.buildDepError "semigroups"))) ++ (if !flags.ghc-lib && (compiler.isGhc && (compiler.version).ge "9.8") && (compiler.isGhc && (compiler.version).lt "9.9")
          then [
