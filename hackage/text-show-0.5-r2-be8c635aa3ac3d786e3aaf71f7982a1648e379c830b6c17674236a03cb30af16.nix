    ...
    components = {
      "library" = {
        depends = (([
          ...
          then [ (hsPkgs.nats) (hsPkgs.void) ] ++ (if flags.transformers-four
            then [ (hsPkgs.transformers) ]
            else [ (hsPkgs.transformers) (hsPkgs.transformers-compat) ])
          else (pkgs.lib).optional (flags.integer-gmp2) (hsPkgs.integer-gmp));
        };
      tests = {
