    ...
    components = {
      "library" = {
        depends = (([
          ...
          then [ (hsPkgs."text" or (buildDepError "text")) ]
          else [
            (hsPkgs."integer-gmp" or (buildDepError "integer-gmp"))
            (hsPkgs."text" or (buildDepError "text"))
            ])) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.7") (hsPkgs."tagged" or (buildDepError "tagged"))) ++ (if compiler.isGhc && (compiler.version).lt "7.9"
          then [
      ...
      tests = {
        "text-show-properties" = {
          depends = (([
            ...
            ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.2" && (compiler.isGhc && (compiler.version).lt "7.6")) (hsPkgs."ghc-prim" or (buildDepError "ghc-prim"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.7") (hsPkgs."tagged" or (buildDepError "tagged"))) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).lt "7.9") ([
            (hsPkgs."nats" or (buildDepError "nats"))
            (hsPkgs."void" or (buildDepError "void"))
            ] ++ (if flags.transformers-four
            then [ (hsPkgs."transformers" or (buildDepError "transformers")) ]
