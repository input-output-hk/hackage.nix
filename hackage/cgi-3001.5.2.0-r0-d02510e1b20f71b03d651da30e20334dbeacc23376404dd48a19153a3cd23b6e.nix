{ system
  ...
  , ... }:
  {
    flags = { xhtml-library-needs-text = true; install-examples = false; };
    package = {
      specVersion = "1.10";
      identifier = { name = "cgi"; version = "3001.5.2.0"; };
    ...
    components = {
      "library" = {
        ...
        ] ++ (if flags.xhtml-library-needs-text
          then [
            (hsPkgs."xhtml" or (errorHandler.buildDepError "xhtml"))
            (hsPkgs."text" or (errorHandler.buildDepError "text"))
          ]
          else [ (hsPkgs."xhtml" or (errorHandler.buildDepError "xhtml")) ]);
        buildable = true;
      };
      exes = {
        "cookie.cgi" = {
          depends = pkgs.lib.optionals (flags.install-examples && !flags.xhtml-library-needs-text) [
            (hsPkgs."base" or (errorHandler.buildDepError "base"))
            (hsPkgs."cgi" or (errorHandler.buildDepError "cgi"))
            (hsPkgs."xhtml" or (errorHandler.buildDepError "xhtml"))
          ];
          buildable = if flags.install-examples && !flags.xhtml-library-needs-text
            then true
            else false;
        };
        "error.cgi" = {
