{ system
  ...
  , ... }:
  {
    flags = {
      ...
      use-wchar32 = true;
      use-imdrawidx32 = true;
    };
    package = {
      specVersion = "3.0";
      identifier = { name = "dear-imgui"; version = "2.3.1"; };
    ...
    components = {
      "library" = {
        ...
        ] ++ pkgs.lib.optional (compiler.isGhc && compiler.version.ge "9.4") (hsPkgs."system-cxx-std-lib" or (errorHandler.buildDepError "system-cxx-std-lib"))) ++ pkgs.lib.optional (flags.opengl2) (hsPkgs."gl" or (errorHandler.buildDepError "gl"))) ++ pkgs.lib.optionals (flags.vulkan) [
        ...
        libs = (pkgs.lib.optional (!(compiler.isGhc && compiler.version.ge "9.4")) (pkgs."stdc++" or (errorHandler.sysDepError "stdc++")) ++ pkgs.lib.optionals (flags.vulkan) (if system.isWindows
      ...
      exes = {
        ...
        "glfw" = {
          depends = [
            (hsPkgs."base" or (errorHandler.buildDepError "base"))
          ] ++ pkgs.lib.optionals (!(!flags.examples || !flags.glfw || !flags.opengl2)) [
            (hsPkgs."base" or (errorHandler.buildDepError "base"))
            (hsPkgs."GLFW-b" or (errorHandler.buildDepError "GLFW-b"))
            (hsPkgs."gl" or (errorHandler.buildDepError "gl"))
            (hsPkgs."dear-imgui" or (errorHandler.buildDepError "dear-imgui"))
        ...
        "image" = {
          ...
          buildable = if !flags.examples || !flags.sdl || !flags.opengl3
            then false
            else true;
        };
        "sdlrenderer" = {
          ...
          buildable = if !flags.examples || !flags.sdl || !flags.sdl-renderer
            then false
            else true;
        };
        "vulkan" = {
