{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "long-double"; version = "0.1.1.1"; };
    ...
    components = {
      "library" = {
        ...
        ] ++ pkgs.lib.optionals (!system.isI386) (pkgs.lib.optionals (!system.isX86_64) (pkgs.lib.optionals (!(system.isArm || system.isAarch64 && (system.isOsx || system.isWindows))) (pkgs.lib.optional (system.isAarch64) (hsPkgs."float128" or (errorHandler.buildDepError "float128")))));
        buildable = if system.isI386
          then true
          else if system.isX86_64
            then true
            else if system.isArm || system.isAarch64 && (system.isOsx || system.isWindows)
              then true
