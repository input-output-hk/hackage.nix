...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "keter"; version = "1.4.3.2"; };
      license = "MIT";
      copyright = "";
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.6") (hsPkgs."ghc-prim" or (buildDepError "ghc-prim"))) ++ (if flags.system-filepath
          then [
            (hsPkgs."fsnotify" or (buildDepError "fsnotify"))
            (hsPkgs."system-filepath" or (buildDepError "system-filepath"))
            ]
          else [ (hsPkgs."fsnotify" or (buildDepError "fsnotify")) ]);
        };
      exes = {
        "keter" = {
          depends = [
