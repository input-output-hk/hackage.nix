{ system
  ...
  , ... }:
  {
    flags = { test = false; mpich14 = false; };
    package = {
      specVersion = "1.6";
      identifier = { name = "haskell-mpi"; version = "1.4.0"; };
    ...
    components = {
      "library" = {
        ...
        libs = if flags.mpich14
          then [
            (pkgs."mpich" or (errorHandler.sysDepError "mpich"))
            (pkgs."opa" or (errorHandler.sysDepError "opa"))
            (pkgs."mpl" or (errorHandler.sysDepError "mpl"))
            ]
          else [
            (pkgs."mpi" or (errorHandler.sysDepError "mpi"))
            (pkgs."open-rte" or (errorHandler.sysDepError "open-rte"))
            (pkgs."open-pal" or (errorHandler.sysDepError "open-pal"))
            ];
        build-tools = [
