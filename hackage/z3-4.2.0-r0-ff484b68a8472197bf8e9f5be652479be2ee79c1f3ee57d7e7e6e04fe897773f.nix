{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "z3"; version = "4.2.0"; };
    ...
    components = {
      "library" = {
        ...
        libs = if system.isOsx || system.isWindows
          then [ (pkgs."z3" or ((hsPkgs.pkgs-errors).sysDepError "z3")) ]
          else [
            (pkgs."gomp" or ((hsPkgs.pkgs-errors).sysDepError "gomp"))
            (pkgs."z3" or ((hsPkgs.pkgs-errors).sysDepError "z3"))
            (pkgs."gomp" or ((hsPkgs.pkgs-errors).sysDepError "gomp"))
            ];
        build-tools = [
