{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "certificate"; version = "0.9.3"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (system.isWindows) (hsPkgs."Win32" or ((hsPkgs.pkgs-errors).buildDepError "Win32"))) ++ (pkgs.lib).optional (system.isOsx) (hsPkgs."process" or ((hsPkgs.pkgs-errors).buildDepError "process"));
        buildable = true;
        };
      exes = {
        "certificate" = {
