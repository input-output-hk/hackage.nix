{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "MagicHaskeller"; version = "0.9.6.7"; };
      ...
      exes = {
        "MagicHaskeller" = {
          depends = ((([
            ...
            (hsPkgs.ghc-paths)
            (hsPkgs.ghc)
            ] ++ (pkgs.lib).optional (!system.isWindows) (hsPkgs.unix)) ++ (pkgs.lib).optional (flags.tfrandom) (hsPkgs.tf-random)) ++ (pkgs.lib).optional (flags.readfile) (hsPkgs.haskell-src)) ++ [
            (hsPkgs.network)
            (hsPkgs.network-uri)
            ];
          };
