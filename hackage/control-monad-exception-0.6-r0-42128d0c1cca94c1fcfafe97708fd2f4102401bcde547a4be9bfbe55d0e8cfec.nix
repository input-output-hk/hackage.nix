{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { mtl = true; monadsfd = true; extensibleexceptions = false; };
    package = {
      specVersion = "1.6";
      identifier = { name = "control-monad-exception"; version = "0.6"; };
      ...
      "library" = {
        depends = (([
          ...
          then [
            ...
            (hsPkgs."base" or ((hsPkgs.pkgs-errors).buildDepError "base"))
            ])) ++ (pkgs.lib).optional (flags.mtl) (hsPkgs."mtl" or ((hsPkgs.pkgs-errors).buildDepError "mtl"))) ++ (pkgs.lib).optionals (flags.monadsfd) [
