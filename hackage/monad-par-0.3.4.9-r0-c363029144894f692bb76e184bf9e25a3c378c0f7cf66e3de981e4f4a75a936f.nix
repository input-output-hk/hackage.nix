{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { chaselev = false; newgeneric = false; };
    package = {
      specVersion = "1.8";
      identifier = { name = "monad-par"; version = "0.3.4.9"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.chaselev) (hsPkgs."chaselev-deque" or ((hsPkgs.pkgs-errors).buildDepError "chaselev-deque"))) ++ (pkgs.lib).optional (flags.newgeneric) (hsPkgs."par-classes" or ((hsPkgs.pkgs-errors).buildDepError "par-classes"));
        buildable = true;
        };
      tests = {
        "test-monad-par" = {
