{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "git-annex"; version = "5.20151208"; };
      ...
      exes = {
        "git-annex" = {
          depends = ((((((((((((((((((((([
            ...
            (hsPkgs.aws)
            ]) ++ (pkgs.lib).optional (flags.webdav) (hsPkgs.DAV)) ++ (pkgs.lib).optionals (flags.assistant) (if system.isLinux && flags.inotify
            ...
            (hsPkgs.gnutls)
            (hsPkgs.xml-types)
            ]) ++ (pkgs.lib).optional (flags.dns) (hsPkgs.dns)) ++ (pkgs.lib).optional (flags.feed) (hsPkgs.feed)) ++ (pkgs.lib).optional (flags.quvi) (hsPkgs.aeson)) ++ (pkgs.lib).optional (flags.tahoe) (hsPkgs.aeson)) ++ (pkgs.lib).optional (flags.torrentparser) (hsPkgs.torrent)) ++ (pkgs.lib).optionals (flags.database) [
            (hsPkgs.esqueleto)
            (hsPkgs.persistent-sqlite)
            (hsPkgs.persistent)
            (hsPkgs.persistent-template)
            ]) ++ (pkgs.lib).optional (flags.concurrentoutput) (hsPkgs.concurrent-output)) ++ (pkgs.lib).optional (flags.ekg) (hsPkgs.ekg);
          };
        };
      };
    }