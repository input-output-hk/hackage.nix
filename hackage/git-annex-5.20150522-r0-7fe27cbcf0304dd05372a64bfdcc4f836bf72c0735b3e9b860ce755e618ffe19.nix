{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "git-annex"; version = "5.20150522"; };
      ...
      exes = {
        "git-annex" = {
          depends = (((((((((((((((((((([
            ...
            else [ (hsPkgs.network) (hsPkgs.network) ])) ++ (if flags.new-time
            then [ (hsPkgs.time) ]
            else [ (hsPkgs.time) (hsPkgs.old-locale) ])) ++ (if system.isWindows
            then [
              (hsPkgs.Win32)
              (hsPkgs.Win32-extras)
              (hsPkgs.unix-compat)
            ...
            (hsPkgs.gnutls)
            (hsPkgs.xml-types)
            ]) ++ (pkgs.lib).optional (flags.dns) (hsPkgs.dns)) ++ (pkgs.lib).optional (flags.feed) (hsPkgs.feed)) ++ (pkgs.lib).optional (flags.quvi) (hsPkgs.aeson)) ++ (pkgs.lib).optional (flags.tahoe) (hsPkgs.aeson)) ++ (pkgs.lib).optional (flags.torrentparser) (hsPkgs.torrent)) ++ (pkgs.lib).optionals (flags.asciiprogress) [
            (hsPkgs.ascii-progress)
            (hsPkgs.terminal-size)
            ]) ++ (pkgs.lib).optional (flags.ekg) (hsPkgs.ekg);
          };
        };
      };
    }