    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.0") (hsPkgs."semigroups" or (buildDepError "semigroups"))) ++ (pkgs.lib).optional (flags.integer-gmp && (compiler.isGhc && (compiler.version).ge "8.0.1")) (hsPkgs."integer-gmp" or (buildDepError "integer-gmp"));
        libs = (pkgs.lib).optional (flags.libgmp) (pkgs."gmp" or (sysDepError "gmp"));
        buildable = true;
        };
      tests = {
        "bitvec-tests" = {
