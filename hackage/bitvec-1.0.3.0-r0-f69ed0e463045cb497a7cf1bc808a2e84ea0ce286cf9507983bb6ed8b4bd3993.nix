{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { integer-gmp = true; libgmp = true; };
    package = {
      specVersion = "1.10";
      identifier = { name = "bitvec"; version = "1.0.3.0"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.0") (hsPkgs."semigroups" or ((hsPkgs.pkgs-errors).buildDepError "semigroups"))) ++ (pkgs.lib).optional (flags.integer-gmp && (compiler.isGhc && (compiler.version).ge "8.0.1")) (hsPkgs."integer-gmp" or ((hsPkgs.pkgs-errors).buildDepError "integer-gmp"));
        libs = (pkgs.lib).optional (flags.libgmp) (pkgs."gmp" or ((hsPkgs.pkgs-errors).sysDepError "gmp"));
        buildable = true;
        };
      tests = {
        "bitvec-tests" = {
