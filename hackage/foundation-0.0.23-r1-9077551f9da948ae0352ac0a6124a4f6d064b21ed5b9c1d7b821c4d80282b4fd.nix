{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.18";
      identifier = { name = "foundation"; version = "0.0.23"; };
    ...
    components = {
      "library" = {
        ...
        libs = (pkgs.lib).optionals (!(compiler.isGhc && (compiler.version).lt "8.0")) ((pkgs.lib).optionals (system.isWindows) ((pkgs.lib).optional (system.isI386) (pkgs."gcc" or ((hsPkgs.pkgs-errors).sysDepError "gcc"))));
        buildable = true;
        };
      tests = {
        "check-foundation" = {
