...
, ... }:
  {
    ...
    components = {
      "library" = {
        ...
        ] ++ pkgs.lib.optional (compiler.isGhc && compiler.version.lt "7.9") (hsPkgs.transformers)) ++ pkgs.lib.optional (flags.integer-gmp && !(compiler.isGhcjs && true)) (hsPkgs.integer-gmp);
      };
      tests = {
        "tests" = {
          depends  = pkgs.lib.optionals (flags.dev) ([
            ...
            (hsPkgs.random)
            (hsPkgs.QuickCheck)
          ] ++ pkgs.lib.optional (flags.integer-gmp && !(compiler.isGhcjs && true)) (hsPkgs.integer-gmp));
        };
      };
      benchmarks = {
        "bench" = {
