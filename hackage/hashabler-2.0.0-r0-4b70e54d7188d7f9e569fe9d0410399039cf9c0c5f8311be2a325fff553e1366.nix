    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.9") (hsPkgs."transformers" or ((hsPkgs.pkgs-errors).buildDepError "transformers"))) ++ (pkgs.lib).optional (flags.integer-gmp && !(compiler.isGhcjs && true)) (hsPkgs."integer-gmp" or ((hsPkgs.pkgs-errors).buildDepError "integer-gmp"));
        buildable = true;
        };
      tests = {
        "tests" = {
          depends = (pkgs.lib).optionals (flags.dev) ([
            ...
            ] ++ (pkgs.lib).optional (flags.integer-gmp && !(compiler.isGhcjs && true)) (hsPkgs."integer-gmp" or ((hsPkgs.pkgs-errors).buildDepError "integer-gmp")));
