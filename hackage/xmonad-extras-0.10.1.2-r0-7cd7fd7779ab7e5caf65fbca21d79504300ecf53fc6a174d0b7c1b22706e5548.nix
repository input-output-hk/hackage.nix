    ...
    components = {
      "library" = {
        depends = ((((([
          ...
          (hsPkgs.libmpd)
          (hsPkgs.bytestring)
          ]) ++ (pkgs.lib).optional (flags.with_regex_posix) (hsPkgs.regex-posix)) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "6.12.1" && flags.with_template_haskell && flags.with_hlist) [
