{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "dependent-map"; version = "0.1.1.2"; };
    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.2") (hsPkgs."dependent-sum" or ((hsPkgs.pkgs-errors).buildDepError "dependent-sum"));
        buildable = true;
        };
      };
    }