{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "hdevtools"; version = "0.1.6.1"; };
      license = "MIT";
      ...
      exes = {
        "hdevtools" = {
          depends = (((([
            ...
            ] ++ (pkgs.lib).optional (compiler.isGhc && ((compiler.version).ge "7.6" && (compiler.version).lt "7.7")) (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.7") (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"))) ++ (pkgs.lib).optionals (compiler.isGhc && ((compiler.version).ge "7.9" && (compiler.version).lt "8.0")) [
            (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"))
            (hsPkgs."bin-package-db" or (errorHandler.buildDepError "bin-package-db"))
            ]) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "8.0") [
            (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"))
            (hsPkgs."ghc-boot" or (errorHandler.buildDepError "ghc-boot"))
            ]) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.2") (hsPkgs."ghc-boot" or (errorHandler.buildDepError "ghc-boot"));
          buildable = true;
          };
        };
      };
