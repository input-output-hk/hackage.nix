    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (flags.ghc-flags) (hsPkgs."ghcflags" or (buildDepError "ghcflags"));
        build-tools = (pkgs.lib).optional (flags.ghc-flags) (hsPkgs.buildPackages.hsinspect or (pkgs.buildPackages.hsinspect or (buildToolDepError "hsinspect")));
        buildable = true;
        };
      tests = {
        "tasty" = {
