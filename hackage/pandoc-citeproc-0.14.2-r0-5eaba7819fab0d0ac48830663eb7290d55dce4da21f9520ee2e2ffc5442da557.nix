    ...
    components = {
      "library" = {
        depends = (((((([
          ...
          (hsPkgs.split)
          (hsPkgs.yaml)
          ] ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8.0")) (hsPkgs.semigroups)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.6") (hsPkgs.ghc-prim)) ++ (pkgs.lib).optional (flags.debug) (hsPkgs.pretty-show)) ++ (pkgs.lib).optional (flags.bibutils) (hsPkgs.hs-bibutils)) ++ (pkgs.lib).optional (flags.embed_data_files) (hsPkgs.file-embed)) ++ (if flags.unicode_collation
