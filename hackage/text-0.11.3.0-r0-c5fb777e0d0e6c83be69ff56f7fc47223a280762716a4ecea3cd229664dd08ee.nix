{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "text"; version = "0.11.3.0"; };
      ...
      "library" = {
        depends = (([
          ...
          then [
            ...
            ])) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "6.9" && (compiler.isGhc && (compiler.version).lt "6.11")) (hsPkgs."integer" or ((hsPkgs.pkgs-errors).buildDepError "integer"));
        buildable = true;
        };
      tests = {
        "tests" = {
