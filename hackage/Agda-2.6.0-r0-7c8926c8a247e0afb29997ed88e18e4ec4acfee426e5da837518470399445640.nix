    ...
    components = {
      "library" = {
        depends = (((((((([
          ...
          (hsPkgs.uri-encode)
          ] ++ (pkgs.lib).optional (flags.enable-cluster-counting) (hsPkgs.text-icu)) ++ (pkgs.lib).optional (system.isWindows) (hsPkgs.Win32)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.6.4" && (compiler.isGhc && (compiler.version).lt "8.6.6")) (hsPkgs.transformers)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.4" && (compiler.isGhc && (compiler.version).lt "8.6.4")) (hsPkgs.transformers)) ++ [
          (hsPkgs.text)
          ]) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.0" && (compiler.isGhc && (compiler.version).lt "8.4")) (hsPkgs.transformers)) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "7.10" && (compiler.isGhc && (compiler.version).lt "8.0")) [
          (hsPkgs.transformers)
          (hsPkgs.filemanip)
          ]) ++ [
          (hsPkgs.zlib)
