    ...
    components = {
      "library" = {
        ...
        libs = if system.isWindows
          then [
            (pkgs."hg3dsfml030" or ((hsPkgs.pkgs-errors).sysDepError "hg3dsfml030"))
            ]
          else [
            (pkgs."hg3dsfml030" or ((hsPkgs.pkgs-errors).sysDepError "hg3dsfml030"))
            (pkgs.":libsfml-audio.so.2" or ((hsPkgs.pkgs-errors).sysDepError ":libsfml-audio.so.2"))
            (pkgs.":libsfml-system.so.2" or ((hsPkgs.pkgs-errors).sysDepError ":libsfml-system.so.2"))
            (pkgs.":libsfml-window.so.2" or ((hsPkgs.pkgs-errors).sysDepError ":libsfml-window.so.2"))
            ];
        buildable = true;
        };
      };
