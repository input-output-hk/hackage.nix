{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "2.0";
      identifier = { name = "log-warper"; version = "1.8.7"; };
      license = "MIT";
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.2.2") (hsPkgs."o-clock" or ((hsPkgs.pkgs-errors).buildDepError "o-clock"))) ++ (pkgs.lib).optional (!system.isWindows) (hsPkgs."unix" or ((hsPkgs.pkgs-errors).buildDepError "unix"));
        buildable = true;
        };
      exes = {
        "play-log" = {
