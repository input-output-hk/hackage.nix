{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "ghc-vis"; version = "0.6.1.1"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ]) ++ (pkgs.lib).optional (flags.full) (hsPkgs."svgcairo" or ((hsPkgs.pkgs-errors).buildDepError "svgcairo"))) ++ (pkgs.lib).optional (flags.sdl) (hsPkgs."SDL" or ((hsPkgs.pkgs-errors).buildDepError "SDL"));
        buildable = true;
        };
      };
    }