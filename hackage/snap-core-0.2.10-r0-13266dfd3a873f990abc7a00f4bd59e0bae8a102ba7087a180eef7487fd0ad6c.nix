    ...
    components = {
      "library" = {
        depends  = ([
          ...
          (hsPkgs.unix-compat)
          (hsPkgs.zlib)
        ] ++ pkgs.lib.optional (flags.testsuite) (hsPkgs.deepseq)) ++ pkgs.lib.optional (!(flags.portable || system.isWindows)) (hsPkgs.bytestring-mmap);
      };
      exes = {
        "snap" = {
          depends  = [
            ...
            (hsPkgs.unix-compat)
            (hsPkgs.zlib)
          ] ++ pkgs.lib.optional (flags.testsuite) (hsPkgs.deepseq);
        };
      };
    };
  }