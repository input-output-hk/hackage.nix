    ...
    components = {
      "library" = {
        depends = ((([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.8") (hsPkgs."atomic-primops" or ((hsPkgs.pkgs-errors).buildDepError "atomic-primops"))) ++ (if flags.network-bytestring
          then [
      ...
      tests = {
        ...
        "spec" = {
          depends = ([
            ...
            ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.8") (hsPkgs."atomic-primops" or ((hsPkgs.pkgs-errors).buildDepError "atomic-primops"))) ++ (pkgs.lib).optionals ((system.isLinux || system.isFreebsd || system.isOsx) && flags.allow-sendfilefd) [
            (hsPkgs."unix" or ((hsPkgs.pkgs-errors).buildDepError "unix"))
