      ...
      "library" = {
        depends = [
          ...
          then [ (hsPkgs.bytestring) (hsPkgs.bytestring-builder) ]
            ...
            (hsPkgs.bytestring)
            ])) ++ (pkgs.lib).optional (flags.containers) (hsPkgs.containers)) ++ (pkgs.lib).optional (flags.deepseq) (hsPkgs.deepseq)) ++ (pkgs.lib).optional (flags.tagged) (hsPkgs.tagged)) ++ (pkgs.lib).optional (flags.text) (hsPkgs.text)) ++ (pkgs.lib).optional (flags.hashable) (hsPkgs.hashable)) ++ (pkgs.lib).optional (flags.hashable && flags.unordered-containers) (hsPkgs.unordered-containers)) ++ (pkgs.lib).optional (flags.transformers) (hsPkgs.transformers));
        };
      };
    }