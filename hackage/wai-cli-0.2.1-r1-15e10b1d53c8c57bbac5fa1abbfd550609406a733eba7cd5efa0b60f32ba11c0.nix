    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (flags.tls) (hsPkgs."warp-tls" or ((hsPkgs.pkgs-errors).buildDepError "warp-tls"))) ++ (pkgs.lib).optional (flags.fastcgi) (hsPkgs."wai-handler-fastcgi" or ((hsPkgs.pkgs-errors).buildDepError "wai-handler-fastcgi"))) ++ (pkgs.lib).optionals (!system.isWindows && flags.unixsockets) [
