{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.4";
      identifier = { name = "HGamer3D-SDL2-Binding"; version = "0.3.0"; };
    ...
    components = {
      "library" = {
        ...
        libs = if system.isWindows
          then [
            (pkgs."hg3dsdl2030" or ((hsPkgs.pkgs-errors).sysDepError "hg3dsdl2030"))
            ]
          else [
            (pkgs."hg3dsdl2030" or ((hsPkgs.pkgs-errors).sysDepError "hg3dsdl2030"))
            (pkgs.":libSDL2-2.0.so.1" or ((hsPkgs.pkgs-errors).sysDepError ":libSDL2-2.0.so.1"))
