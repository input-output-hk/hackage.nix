{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "selda"; version = "0.1.9.0"; };
      license = "MIT";
      copyright = "";
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.11") (hsPkgs."transformers" or ((hsPkgs.pkgs-errors).buildDepError "transformers"))) ++ (pkgs.lib).optional (!flags.haste && flags.localcache) (hsPkgs."psqueues" or ((hsPkgs.pkgs-errors).buildDepError "psqueues"));
        buildable = true;
        };
      };
    }