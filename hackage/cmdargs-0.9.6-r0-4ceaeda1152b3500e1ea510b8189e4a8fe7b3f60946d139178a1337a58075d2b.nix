{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { testprog = true; quotation = true; };
    package = {
      specVersion = "1.6";
      identifier = { name = "cmdargs"; version = "0.9.6"; };
    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (flags.quotation) (hsPkgs."template-haskell" or ((hsPkgs.pkgs-errors).buildDepError "template-haskell"));
        buildable = true;
        };
      exes = {
        "cmdargs" = {
