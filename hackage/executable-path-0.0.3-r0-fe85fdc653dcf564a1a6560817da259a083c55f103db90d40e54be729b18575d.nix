...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.2";
      identifier = { name = "executable-path"; version = "0.0.3"; };
    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && true) (hsPkgs."directory" or (buildDepError "directory"))) ++ (pkgs.lib).optional (system.isLinux) (hsPkgs."unix" or (buildDepError "unix"))) ++ (pkgs.lib).optionals (system.isFreebsd) [
