    ...
    components = {
      "library" = {
        depends = ([
          ...
          then [
            (hsPkgs."regex-posix-clib" or (buildDepError "regex-posix-clib"))
            ]
          else (pkgs.lib).optional (system.isWindows) (hsPkgs."base" or (buildDepError "base")))) ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8")) (hsPkgs."fail" or (buildDepError "fail"));
        buildable = true;
        };
      };
    }