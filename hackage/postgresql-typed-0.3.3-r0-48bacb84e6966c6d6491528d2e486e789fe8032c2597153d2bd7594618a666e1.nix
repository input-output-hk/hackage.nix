    ...
    components = {
      "library" = {
        depends = (([
          ...
          else ((pkgs.lib).optional (flags.text) (hsPkgs.text) ++ (pkgs.lib).optional (flags.uuid) (hsPkgs.uuid)) ++ (pkgs.lib).optional (flags.scientific) (hsPkgs.scientific))) ++ (pkgs.lib).optionals (flags.aeson) [
          (hsPkgs.aeson)
          (hsPkgs.attoparsec)
          ];
        };
