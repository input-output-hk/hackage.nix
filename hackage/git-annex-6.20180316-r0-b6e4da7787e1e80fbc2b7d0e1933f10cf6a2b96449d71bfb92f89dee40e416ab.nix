      ...
      exes = {
        "git-annex" = {
          depends = ((((((((((((([
            ...
            else [ (hsPkgs.unix) ])) ++ (pkgs.lib).optionals (flags.s3) [
            (hsPkgs.conduit)
            (hsPkgs.conduit-extra)
            (hsPkgs.aws)
            ]) ++ (pkgs.lib).optional (flags.webdav) (hsPkgs.DAV)) ++ (pkgs.lib).optionals (flags.assistant && !system.isSolaris && !system.isHurd) [
            (hsPkgs.dns)
            (hsPkgs.mountpoints)
