    ...
    components = {
      "library" = {
        depends = ((([
          ...
          (hsPkgs.basement)
          (hsPkgs.ghc-prim)
          ] ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).lt "8.0")) (hsPkgs.base)) ++ (pkgs.lib).optional (system.isWindows) (hsPkgs.Win32)) ++ (pkgs.lib).optional (compiler.isGhc && true && flags.integer-gmp) (hsPkgs.integer-gmp)) ++ (pkgs.lib).optional (flags.support_deepseq) (hsPkgs.deepseq);
