    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ]) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4.0" && !flags.profiling) (hsPkgs."ghc-datasize" or ((hsPkgs.pkgs-errors).buildDepError "ghc-datasize"))) ++ (if compiler.isGhcjs && true
          then [
