...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "hanabi-dealer"; version = "0.8.0.0"; };
    ...
    components = {
      "library" = {
        depends = (pkgs.lib).optionals (!flags.jsaddle) ((([
          ...
          ] ++ (pkgs.lib).optionals (compiler.isGhcjs && true || flags.server) ((pkgs.lib).optional (flags.th) (hsPkgs."template-haskell" or (buildDepError "template-haskell")))) ++ (pkgs.lib).optionals (!(compiler.isGhcjs && true) && flags.server) (([
      ...
      exes = {
        "hanabiq" = {
          depends = (pkgs.lib).optionals (!(!flags.jsaddle)) ([
            ...
            (hsPkgs."http-types" or (buildDepError "http-types"))
            (hsPkgs."network-uri" or (buildDepError "network-uri"))
            ] ++ (pkgs.lib).optional (flags.tfrandom) (hsPkgs."tf-random" or (buildDepError "tf-random")));
          buildable = if !flags.jsaddle then false else true;
          };
        "hanabib" = {
