{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { bytestring = true; doc = true; text = true; };
    package = {
      specVersion = "1.6";
      identifier = { name = "to-string-instances"; version = "0.2"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (flags.bytestring) (hsPkgs."bytestring" or ((hsPkgs.pkgs-errors).buildDepError "bytestring"))) ++ (pkgs.lib).optional (flags.doc) (hsPkgs."pretty" or ((hsPkgs.pkgs-errors).buildDepError "pretty"))) ++ (pkgs.lib).optional (flags.text) (hsPkgs."text" or ((hsPkgs.pkgs-errors).buildDepError "text"));
        buildable = true;
        };
      };
    }