{ system
  ...
  , ... }:
  {
    flags = { developer = false; ghc-boot = true; };
    package = {
      specVersion = "1.10";
      identifier = { name = "text-show-instances"; version = "3.3"; };
    ...
    components = {
      "library" = {
        ...
        ] ++ pkgs.lib.optional (flags.ghc-boot) (hsPkgs."ghc-boot" or (errorHandler.buildDepError "ghc-boot"))) ++ (if system.isWindows
      ...
      tests = {
        "spec" = {
          depends = (([
            ...
            (hsPkgs."vector" or (errorHandler.buildDepError "vector"))
            (hsPkgs."xhtml" or (errorHandler.buildDepError "xhtml"))
          ] ++ pkgs.lib.optional (!flags.developer) (hsPkgs."text-show-instances" or (errorHandler.buildDepError "text-show-instances"))) ++ pkgs.lib.optional (flags.ghc-boot) (hsPkgs."ghc-boot" or (errorHandler.buildDepError "ghc-boot"))) ++ (if system.isWindows
            then [ (hsPkgs."Win32" or (errorHandler.buildDepError "Win32")) ]
            else [
              (hsPkgs."terminfo" or (errorHandler.buildDepError "terminfo"))
              (hsPkgs."unix" or (errorHandler.buildDepError "unix"))
