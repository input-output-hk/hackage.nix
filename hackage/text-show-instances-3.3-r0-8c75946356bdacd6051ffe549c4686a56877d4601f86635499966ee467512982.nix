    ...
    components = {
      "library" = {
        depends = ([
          ...
          (hsPkgs.vector)
          (hsPkgs.xhtml)
          ] ++ (pkgs.lib).optional (flags.ghc-boot) (hsPkgs.ghc-boot)) ++ (if system.isWindows
          then [ (hsPkgs.Win32) ]
      ...
      tests = {
        "spec" = {
          depends = (([
            ...
            (hsPkgs.vector)
            (hsPkgs.xhtml)
            ] ++ (pkgs.lib).optional (!flags.developer) (hsPkgs.text-show-instances)) ++ (pkgs.lib).optional (flags.ghc-boot) (hsPkgs.ghc-boot)) ++ (if system.isWindows
            then [ (hsPkgs.Win32) ]
            else [ (hsPkgs.terminfo) (hsPkgs.unix) ]);
          };
        };
