  ...
  , ... }:
  {
    ...
    components = {
      "library" = {
        ...
        build-tools = (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.10") (hsPkgs.buildPackages.cpphs.components.exes.cpphs or (pkgs.buildPackages.cpphs or (errorHandler.buildToolDepError "cpphs:cpphs")));
        buildable = true;
        };
      tests = {
        "test" = {
