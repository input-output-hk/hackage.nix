{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.4";
      identifier = { name = "HGamer3D-Enet-Binding"; version = "0.5.0"; };
    ...
    components = {
      "library" = {
        ...
        libs = if system.isWindows
          then [
            (pkgs."hg3denet050" or ((hsPkgs.pkgs-errors).sysDepError "hg3denet050"))
            ]
          else [
            (pkgs."hg3denet050" or ((hsPkgs.pkgs-errors).sysDepError "hg3denet050"))
            (pkgs."enet" or ((hsPkgs.pkgs-errors).sysDepError "enet"))
            ];
        buildable = true;
        };
      };
