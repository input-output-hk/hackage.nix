    ...
    components = {
      "library" = {
        depends = (([
          ...
          (hsPkgs.ghc-heap)
          (hsPkgs.ghci)
          ] ++ (pkgs.lib).optional (!(system.isLinux && system.isX86_64 && (compiler.isGhc && (compiler.version).eq "8.6.5"))) (hsPkgs.base)) ++ (pkgs.lib).optional (!flags.buildable) (hsPkgs.base)) ++ (if system.isWindows
          then [ (hsPkgs.Win32) ]
          else [
            (hsPkgs.unix)
