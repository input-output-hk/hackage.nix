    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (flags.quickcheck) (hsPkgs."QuickCheck" or ((hsPkgs.pkgs-errors).buildDepError "QuickCheck"))) ++ (pkgs.lib).optional (flags.hunit) (hsPkgs."HUnit" or ((hsPkgs.pkgs-errors).buildDepError "HUnit"))) ++ (if flags.splitbase
          then [
