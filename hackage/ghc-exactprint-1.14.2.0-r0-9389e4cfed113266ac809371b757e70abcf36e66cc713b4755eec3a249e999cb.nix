{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "2.4";
      identifier = { name = "ghc-exactprint"; version = "1.14.2.0"; };
    ...
    components = {
      ...
      exes = {
        "roundtrip" = {
          depends = pkgs.lib.optionals (compiler.isGhc && compiler.version.ge "9.14" && flags.roundtrip) [
          ...
          buildable = if compiler.isGhc && compiler.version.ge "9.14" && flags.roundtrip
            then true
            else false;
        };
        "static" = {
