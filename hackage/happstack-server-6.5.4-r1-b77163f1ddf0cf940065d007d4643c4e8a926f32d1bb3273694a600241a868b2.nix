{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "happstack-server"; version = "6.5.4"; };
    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ] ++ (pkgs.lib).optional (flags.https) (hsPkgs."HsOpenSSL" or ((hsPkgs.pkgs-errors).buildDepError "HsOpenSSL"))) ++ (if flags.network_2_2_3
          then [
        ...
        libs = (pkgs.lib).optionals (flags.https) [
          (pkgs."cryptopp" or ((hsPkgs.pkgs-errors).sysDepError "cryptopp"))
