    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (!flags.no_arbitrary) (hsPkgs."QuickCheck" or ((hsPkgs.pkgs-errors).buildDepError "QuickCheck"))) ++ (pkgs.lib).optional (!flags.no_comonad) (hsPkgs."comonad" or ((hsPkgs.pkgs-errors).buildDepError "comonad"))) ++ (pkgs.lib).optionals (flags.tutorial) [
