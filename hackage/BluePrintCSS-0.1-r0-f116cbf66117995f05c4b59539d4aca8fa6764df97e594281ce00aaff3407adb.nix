    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.blaze) (hsPkgs."blaze-html" or ((hsPkgs.pkgs-errors).buildDepError "blaze-html"))) ++ (pkgs.lib).optional (flags.hsx) (hsPkgs."hsx" or ((hsPkgs.pkgs-errors).buildDepError "hsx"));
        buildable = true;
        };
      };
    }