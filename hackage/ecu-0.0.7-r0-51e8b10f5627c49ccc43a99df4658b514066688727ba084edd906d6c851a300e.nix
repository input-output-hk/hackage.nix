{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    components = {
      exes = {
        "tovcd" = {
          depends = [ (hsPkgs.base) (hsPkgs.bytestring) (hsPkgs.vcd) ];
          };
        "decomp" = { depends = [ (hsPkgs.base) (hsPkgs.bytestring) ]; };
        "cansend" = {
          depends = [ (hsPkgs.base) ];
          libs = [ (pkgs."canlib") ];
          };
        ...
        "ccp" = { depends = [ (hsPkgs.base) ]; libs = [ (pkgs."canlib") ]; };
        "probe" = {
          depends = [ (hsPkgs.base) (hsPkgs.digest) (hsPkgs.vcd) ];
          libs = [ (pkgs."canlib") ];
          };
        "toesb" = {
          depends = [ (hsPkgs.base) (hsPkgs.bytestring) (hsPkgs.digest) ];
          };
        "parsedbc" = { depends = [ (hsPkgs.base) ]; };
        "commit" = { depends = [ (hsPkgs.base) ]; };
        "git2cc" = {
