  ...
  , ... }:
  {
    ...
    components = {
      "library" = {
        ...
        build-tools = (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "7.10.1" && !system.isWindows)) (hsPkgs.buildPackages.cpphs or (pkgs.buildPackages.cpphs or (errorHandler.buildToolDepError "cpphs")));
        buildable = true;
        };
      };
    }