{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    components = {
      "library" = {
        depends = [ (hsPkgs.base) (hsPkgs.OpenGLRaw) ];
        libs = if system.isWindows && flags.usenativewindowslibraries
          then [ (pkgs."glu32") ]
          else (pkgs.lib).optional (!system.isOsx) (pkgs."GLU");
        frameworks = (pkgs.lib).optionals (!(system.isWindows && flags.usenativewindowslibraries)) ((pkgs.lib).optional (system.isOsx) (pkgs."OpenGL"));
        };
      };
    }