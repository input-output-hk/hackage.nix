{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "text-show"; version = "0.7.0.1"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.7") (hsPkgs."tagged" or (errorHandler.buildDepError "tagged"))) ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "7.8")) (hsPkgs."bytestring-builder" or (errorHandler.buildDepError "bytestring-builder"))) ++ (if compiler.isGhc && (compiler.version).lt "7.9"
          then [
      ...
      tests = {
        "text-show-properties" = {
          depends = (([
            ...
            ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.7") (hsPkgs."tagged" or (errorHandler.buildDepError "tagged"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.8") (hsPkgs."bytestring-builder" or (errorHandler.buildDepError "bytestring-builder"))) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).lt "7.9") [
            (hsPkgs."nats" or (errorHandler.buildDepError "nats"))
            (hsPkgs."transformers" or (errorHandler.buildDepError "transformers"))
