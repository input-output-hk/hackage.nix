{ system
  ...
  , ... }:
  {
    flags = { exceptions = true; mtl = true; unliftio = true; };
    package = {
      specVersion = "3.0";
      identifier = { name = "deriving-trans"; version = "0.5.1.0"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (flags.exceptions) (hsPkgs."exceptions" or (errorHandler.buildDepError "exceptions"))) ++ (pkgs.lib).optional (flags.mtl) (hsPkgs."mtl" or (errorHandler.buildDepError "mtl"))) ++ (pkgs.lib).optional (flags.unliftio) (hsPkgs."unliftio-core" or (errorHandler.buildDepError "unliftio-core"));
        buildable = true;
        };
      };
    }