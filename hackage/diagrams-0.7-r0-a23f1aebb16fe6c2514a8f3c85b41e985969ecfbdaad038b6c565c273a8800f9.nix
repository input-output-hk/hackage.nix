{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { cairo = false; svg = true; ps = false; };
    package = {
      specVersion = "1.10";
      identifier = { name = "diagrams"; version = "0.7"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (flags.cairo) (hsPkgs."diagrams-cairo" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-cairo"))) ++ (pkgs.lib).optional (flags.svg) (hsPkgs."diagrams-svg" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-svg"))) ++ (pkgs.lib).optional (flags.ps) (hsPkgs."diagrams-postscript" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-postscript"));
        buildable = true;
        };
      };
    }