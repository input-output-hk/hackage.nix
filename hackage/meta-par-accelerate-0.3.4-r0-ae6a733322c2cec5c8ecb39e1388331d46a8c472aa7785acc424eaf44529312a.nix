...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "meta-par-accelerate"; version = "0.3.4"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.newaccelerate && flags.cuda) (hsPkgs."accelerate-cuda" or (buildDepError "accelerate-cuda"))) ++ (if flags.newaccelerate
          then [
            (hsPkgs."accelerate" or (buildDepError "accelerate"))
            (hsPkgs."accelerate-io" or (buildDepError "accelerate-io"))
            ]
          else [ (hsPkgs."accelerate" or (buildDepError "accelerate")) ]);
        };
      };
    }