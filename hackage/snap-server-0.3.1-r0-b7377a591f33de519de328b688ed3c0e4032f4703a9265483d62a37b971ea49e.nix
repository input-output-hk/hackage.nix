{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "snap-server"; version = "0.3.1"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (!(flags.portable || system.isWindows)) (hsPkgs."unix" or ((hsPkgs.pkgs-errors).buildDepError "unix"))) ++ (pkgs.lib).optional (flags.libev) (hsPkgs."hlibev" or ((hsPkgs.pkgs-errors).buildDepError "hlibev"));
        libs = (pkgs.lib).optional (flags.gnutls) (pkgs."gnutls" or ((hsPkgs.pkgs-errors).sysDepError "gnutls"));
        buildable = true;
        };
      };
    }