{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.4";
      identifier = { name = "Takusen"; version = "0.8.2"; };
    ...
    components = {
      "library" = {
        ...
        build-tools = (((pkgs.lib).optional (flags.odbc) (hsPkgs.buildPackages.odbcconf or (pkgs.buildPackages.odbcconf or (errorHandler.buildToolDepError "odbcconf"))) ++ (pkgs.lib).optional (flags.oracle) (hsPkgs.buildPackages.sqlplus or (pkgs.buildPackages.sqlplus or (errorHandler.buildToolDepError "sqlplus")))) ++ (pkgs.lib).optional (flags.postgres) (hsPkgs.buildPackages.pg_config or (pkgs.buildPackages.pg_config or (errorHandler.buildToolDepError "pg_config")))) ++ (pkgs.lib).optional (flags.sqlite) (hsPkgs.buildPackages.sqlite3 or (pkgs.buildPackages.sqlite3 or (errorHandler.buildToolDepError "sqlite3")));
        buildable = true;
        };
      };
    }