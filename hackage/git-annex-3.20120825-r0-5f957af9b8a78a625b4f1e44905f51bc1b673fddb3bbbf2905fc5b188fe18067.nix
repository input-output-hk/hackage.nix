{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "git-annex"; version = "3.20120825"; };
      ...
      exes = {
        "git-annex" = {
          depends = (([
            ...
            ] ++ (pkgs.lib).optional (flags.s3) (hsPkgs."hS3" or (errorHandler.buildDepError "hS3"))) ++ (pkgs.lib).optional (flags.assistant) (hsPkgs."stm" or (errorHandler.buildDepError "stm"))) ++ (pkgs.lib).optional (system.isLinux && flags.inotify) (hsPkgs."hinotify" or (errorHandler.buildDepError "hinotify"));
          buildable = true;
          };
        };
      tests = {
