    ...
    components = {
      "library" = {
        ...
        build-tools = [
          ...
          ] ++ (pkgs.lib).optional (!system.isOsx && !system.isWindows) (hsPkgs.buildPackages.pkg-config or (pkgs.buildPackages.pkg-config or (errorHandler.buildToolDepError "pkg-config")));
        buildable = true;
        };
      exes = {
        "fltkhs-fluidtohs" = {
