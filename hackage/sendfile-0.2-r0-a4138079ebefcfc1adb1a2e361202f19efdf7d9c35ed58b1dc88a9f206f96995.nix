    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optionals (!(system.isWindows && !flags.portable)) ((pkgs.lib).optional (!(system.isLinux && !flags.portable)) (hsPkgs."bytestring" or (buildDepError "bytestring")));
        libs = (pkgs.lib).optionals (system.isWindows && !flags.portable) [
          (pkgs."kernel32" or (sysDepError "kernel32"))
          (pkgs."mswsock" or (sysDepError "mswsock"))
          ];
        };
      };
    }