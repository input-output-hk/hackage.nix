    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optionals (!(system.isWindows && !flags.portable)) ((pkgs.lib).optional (!(system.isLinux && !flags.portable)) (hsPkgs."bytestring" or ((hsPkgs.pkgs-errors).buildDepError "bytestring")));
