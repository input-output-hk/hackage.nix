...
, ... }:
  {
    ...
    components = {
      "library" = {
        ...
        ] ++ pkgs.lib.optional (compiler.isGhc && compiler.version.ge "7.8") (hsPkgs.atomic-primops)) ++ (if flags.network-bytestring
          then [
            (hsPkgs.network)
      ...
      tests = {
        ...
        "spec" = {
          depends = ([
            ...
            (hsPkgs.streaming-commons)
            (hsPkgs.async)
          ] ++ pkgs.lib.optional (compiler.isGhc && compiler.version.ge "7.8") (hsPkgs.atomic-primops)) ++ pkgs.lib.optionals ((system.isLinux || system.isFreebsd || system.isOsx) && flags.allow-sendfilefd) [
            (hsPkgs.unix)
            (hsPkgs.hashable)
            (hsPkgs.http-date)
          ];
