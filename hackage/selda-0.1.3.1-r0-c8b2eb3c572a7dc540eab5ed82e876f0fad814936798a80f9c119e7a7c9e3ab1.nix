...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "selda"; version = "0.1.3.1"; };
      license = "MIT";
      copyright = "";
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.11") (hsPkgs."transformers" or (buildDepError "transformers"))) ++ (pkgs.lib).optionals (!flags.haste && flags.localcache) [
