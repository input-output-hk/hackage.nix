{ system
  ...
  , ... }:
  {
    flags = { developer = false; bytestring = true; old-time = true; };
    package = {
      specVersion = "1.10";
      identifier = { name = "fudgets"; version = "0.18.4"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.bytestring) (hsPkgs."bytestring" or (errorHandler.buildDepError "bytestring"))) ++ (pkgs.lib).optional (flags.old-time) (hsPkgs."old-time" or (errorHandler.buildDepError "old-time"));
        libs = [
