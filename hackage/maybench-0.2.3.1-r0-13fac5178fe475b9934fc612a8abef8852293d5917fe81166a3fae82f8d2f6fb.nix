{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.2";
      identifier = { name = "maybench"; version = "0.2.3.1"; };
    ...
    components = {
      "library" = {
        depends = [ (hsPkgs.benchpress) ] ++ (if flags.splitbase
          then [ (hsPkgs.base) (hsPkgs.process) (hsPkgs.old-time) ]
          else [ (hsPkgs.base) (hsPkgs.time) (hsPkgs.mtl) ]);
        };
      exes = {
        "maybench" = {
          depends = [ (hsPkgs.benchpress) ] ++ (if flags.splitbase
