{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "ghc-typelits-natnormalise"; version = "0.7.10"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ])) ++ pkgs.lib.optional (compiler.isGhc && compiler.version.ge "9.4" && (compiler.isGhc && compiler.version.lt "9.12")) (hsPkgs."template-haskell" or (errorHandler.buildDepError "template-haskell"));
        buildable = true;
      };
      tests = {
        "unit-tests" = {
