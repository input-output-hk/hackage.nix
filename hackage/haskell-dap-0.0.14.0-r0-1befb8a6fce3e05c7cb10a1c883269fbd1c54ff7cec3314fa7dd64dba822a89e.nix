{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.12";
      identifier = { name = "haskell-dap"; version = "0.0.14.0"; };
    ...
    components = {
      ...
      exes = {
        "haskell-dap" = {
          depends = ((pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.8.1") (hsPkgs."base" or ((hsPkgs.pkgs-errors).buildDepError "base")) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "8.0.0" && (compiler.isGhc && (compiler.version).lt "8.8.0")) [
