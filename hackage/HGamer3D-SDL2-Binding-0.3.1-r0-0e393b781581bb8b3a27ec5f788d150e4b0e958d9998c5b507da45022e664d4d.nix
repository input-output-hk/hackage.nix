{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.4";
      identifier = { name = "HGamer3D-SDL2-Binding"; version = "0.3.1"; };
    ...
    components = {
      "library" = {
        ...
        libs = if system.isWindows
          then [
            (pkgs."hg3dsdl2031" or ((hsPkgs.pkgs-errors).sysDepError "hg3dsdl2031"))
            ]
          else [
            (pkgs."hg3dsdl2031" or ((hsPkgs.pkgs-errors).sysDepError "hg3dsdl2031"))
            (pkgs."SDL2-2.0" or ((hsPkgs.pkgs-errors).sysDepError "SDL2-2.0"))
