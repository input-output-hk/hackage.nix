{ system
  ...
  , ... }:
  {
    flags = { doctests = false; executable = false; icu-benchmark = false; };
    package = {
      specVersion = "2.2";
      identifier = { name = "unicode-collation"; version = "0.1.3.7"; };
    ...
    components = {
      ...
      benchmarks = {
        "benchmark" = {
          ...
          buildable = if flags.icu-benchmark then true else false;
        };
        "bench-sortkey" = {
          depends = [
            (hsPkgs."base" or (errorHandler.buildDepError "base"))
