{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    components = {
      "library" = {
        ...
        build-tools = ((pkgs.lib).optional (flags.oracle) (hsPkgs.buildPackages.sqlplus or (pkgs.buildPackages.sqlplus)) ++ (pkgs.lib).optional (flags.postgres) (hsPkgs.buildPackages.pg_config or (pkgs.buildPackages.pg_config))) ++ (pkgs.lib).optional (flags.sqlite) (hsPkgs.buildPackages.sqlite3 or (pkgs.buildPackages.sqlite3));
        };
      exes = {
        "takusen_tests" = {
          depends = (([
            ...
            (hsPkgs.time)
            (hsPkgs.QuickCheck)
            ] ++ (pkgs.lib).optional (!(!flags.buildtests)) (hsPkgs.Takusen)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "6.8") (hsPkgs.old-time)) ++ [
            (hsPkgs.base)
            ];
          };
        "miniunit_tests" = {
          depends = [ (hsPkgs.base) (hsPkgs.mtl) ] ++ [ (hsPkgs.base) ];
          };
        };
      };
    }