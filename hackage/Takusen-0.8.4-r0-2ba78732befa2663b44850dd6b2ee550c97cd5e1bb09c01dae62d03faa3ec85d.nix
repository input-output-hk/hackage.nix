      ...
      exes = {
        "takusen_tests" = {
          depends = (([
            ...
            ] ++ (pkgs.lib).optional (!(!flags.buildtests)) (hsPkgs."Takusen" or ((hsPkgs.pkgs-errors).buildDepError "Takusen"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "6.8") (hsPkgs."old-time" or ((hsPkgs.pkgs-errors).buildDepError "old-time"))) ++ [
            (hsPkgs."base" or ((hsPkgs.pkgs-errors).buildDepError "base"))
            ];
          buildable = if !flags.buildtests then false else true;
          };
