{ system
  ...
  , ... }:
  {
    flags = { no-charts = false; no-colors = false; };
    package = {
      specVersion = "1.18";
      identifier = { name = "bench-show"; version = "0.3.2"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optionals (!flags.no-charts) [
          (hsPkgs."Chart" or (errorHandler.buildDepError "Chart"))
          (hsPkgs."Chart-diagrams" or (errorHandler.buildDepError "Chart-diagrams"))
          ]) ++ (pkgs.lib).optional (!flags.no-colors) (hsPkgs."ansi-wl-pprint" or (errorHandler.buildDepError "ansi-wl-pprint"));
        buildable = true;
        };
      exes = {
        "bench-show" = {
      ...
      tests = {
        "test" = {
          ...
          buildable = if flags.no-charts then false else true;
          };
        "doc" = {
          depends = [
            (hsPkgs."bench-show" or (errorHandler.buildDepError "bench-show"))
