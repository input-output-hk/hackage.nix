{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "2.0";
      identifier = { name = "fltkhs"; version = "0.7.0.0"; };
      license = "MIT";
    ...
    components = {
      "library" = {
        ...
        build-tools = [
          ...
          ] ++ (pkgs.lib).optional (!system.isOsx && !system.isWindows) (hsPkgs.buildPackages.pkg-config or (pkgs.buildPackages.pkg-config or ((hsPkgs.pkgs-errors).buildToolDepError "pkg-config")));
        buildable = true;
        };
      exes = {
        "fltkhs-fluidtohs" = {
