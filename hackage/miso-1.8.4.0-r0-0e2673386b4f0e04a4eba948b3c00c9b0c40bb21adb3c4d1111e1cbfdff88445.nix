{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.22";
      identifier = { name = "miso"; version = "1.8.4.0"; };
    ...
    components = {
      "library" = {
        ...
        ] ++ (if compiler.isGhcjs && true || system.isJavaScript
          then [
      ...
      exes = {
        "tests" = {
          depends = pkgs.lib.optionals (!(!(compiler.isGhcjs && true || system.isJavaScript) || !flags.tests)) [
            (hsPkgs."aeson" or (errorHandler.buildDepError "aeson"))
            (hsPkgs."base" or (errorHandler.buildDepError "base"))
            (hsPkgs."bytestring" or (errorHandler.buildDepError "bytestring"))
            (hsPkgs."ghcjs-base" or (errorHandler.buildDepError "ghcjs-base"))
          ...
          buildable = if !(compiler.isGhcjs && true || system.isJavaScript) || !flags.tests
            then false
            else true;
        };
      };
