{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "mtl-compat"; version = "0.2.1.3"; };
    ...
    components = {
      "library" = {
        depends = [ (hsPkgs.base) ] ++ (if flags.two-point-one
          then [ (hsPkgs.mtl) (hsPkgs.transformers-compat) ]
          else if flags.two-point-two
            then [ (hsPkgs.mtl) (hsPkgs.transformers) ]
            else [ (hsPkgs.mtl) ]);
        };
      };
    }