    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (!(system.isWindows || flags.allow-non-windows)) (hsPkgs."base" or ((hsPkgs.pkgs-errors).buildDepError "base"));
        libs = [
          (pkgs."dnsapi" or ((hsPkgs.pkgs-errors).sysDepError "dnsapi"))
          ];
        build-tools = [
