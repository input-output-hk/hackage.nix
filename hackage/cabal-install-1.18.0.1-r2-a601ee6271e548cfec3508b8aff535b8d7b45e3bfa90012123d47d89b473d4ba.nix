{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "cabal-install"; version = "1.18.0.1"; };
      ...
      tests = {
        "unit-tests" = {
          depends = ([
            ...
            ] ++ (pkgs.lib).optional (flags.old-directory) (hsPkgs."old-time" or ((hsPkgs.pkgs-errors).buildDepError "old-time"))) ++ (if system.isWindows
            then [
              (hsPkgs."Win32" or ((hsPkgs.pkgs-errors).buildDepError "Win32"))
              ]
            else [
