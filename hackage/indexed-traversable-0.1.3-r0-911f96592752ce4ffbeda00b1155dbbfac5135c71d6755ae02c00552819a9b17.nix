{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.12";
      identifier = { name = "indexed-traversable"; version = "0.1.3"; };
    ...
    components = {
      "library" = {
        depends = ((((([
          ...
          ]) ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "9.6")) (hsPkgs."foldable1-classes-compat" or (errorHandler.buildDepError "foldable1-classes-compat"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.0" && (compiler.isGhc && (compiler.version).lt "7.6")) (hsPkgs."ghc-prim" or (errorHandler.buildDepError "ghc-prim"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.0" && (compiler.isGhc && (compiler.version).lt "7.2")) (hsPkgs."generic-deriving" or (errorHandler.buildDepError "generic-deriving"));
        buildable = true;
        };
      };
    }