    ...
    components = {
      "library" = {
        depends = ((((([
          ...
          (hsPkgs.yaml)
          (hsPkgs.pandoc)
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.6") (hsPkgs.ghc-prim)) ++ (pkgs.lib).optional (flags.bibutils) (hsPkgs.hs-bibutils)) ++ (pkgs.lib).optionals (flags.network) [
          (hsPkgs.network)
          (hsPkgs.HTTP)
          ]) ++ (if flags.hexpat
          then [ (hsPkgs.hexpat) ]
