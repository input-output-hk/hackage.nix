{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "happstack-server"; version = "6.5.6"; };
    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ] ++ (pkgs.lib).optional (flags.https) (hsPkgs."HsOpenSSL" or (errorHandler.buildDepError "HsOpenSSL"))) ++ (if flags.network_2_2_3
        ...
        libs = (pkgs.lib).optionals (flags.https) [
          (pkgs."cryptopp" or (errorHandler.sysDepError "cryptopp"))
