...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "unbound-generics"; version = "0.3.3"; };
      ...
      benchmarks = {
        "benchmark-unbound-generics" = {
          depends = ([
            ...
            (hsPkgs."deepseq" or (buildDepError "deepseq"))
            (hsPkgs."unbound-generics" or (buildDepError "unbound-generics"))
            ] ++ (pkgs.lib).optional (compiler.isGhc && false) (hsPkgs."unix" or (buildDepError "unix"))) ++ (if compiler.isGhc && false || compiler.isGhc && false
            then [
