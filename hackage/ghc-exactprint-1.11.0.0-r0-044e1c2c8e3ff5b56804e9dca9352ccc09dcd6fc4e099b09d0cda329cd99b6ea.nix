{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "2.4";
      identifier = { name = "ghc-exactprint"; version = "1.11.0.0"; };
    ...
    components = {
      "library" = {
        ...
        buildable = if compiler.isGhc && compiler.version.lt "9.12"
          then false
          else true;
      };
      exes = {
        "roundtrip" = {
          depends = pkgs.lib.optionals (compiler.isGhc && compiler.version.ge "9.12" && flags.roundtrip) [
          ...
          buildable = if compiler.isGhc && compiler.version.ge "9.12" && flags.roundtrip
            then true
            else false;
        };
        "static" = {
