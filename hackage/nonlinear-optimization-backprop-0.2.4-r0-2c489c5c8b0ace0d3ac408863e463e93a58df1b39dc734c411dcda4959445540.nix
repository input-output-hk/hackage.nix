    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") (hsPkgs."vinyl" or (buildDepError "vinyl"));
        buildable = true;
        };
      exes = {
        "LinearRegression" = {
          depends = [
            ...
            (hsPkgs."nonlinear-optimization-backprop" or (buildDepError "nonlinear-optimization-backprop"))
            (hsPkgs."vector" or (buildDepError "vector"))
            ];
