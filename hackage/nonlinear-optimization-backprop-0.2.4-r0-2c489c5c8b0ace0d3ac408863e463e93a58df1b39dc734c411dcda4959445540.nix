    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") (hsPkgs."vinyl" or ((hsPkgs.pkgs-errors).buildDepError "vinyl"));
        buildable = true;
        };
      exes = {
        "LinearRegression" = {
          depends = [
            ...
            (hsPkgs."nonlinear-optimization-backprop" or ((hsPkgs.pkgs-errors).buildDepError "nonlinear-optimization-backprop"))
            (hsPkgs."vector" or ((hsPkgs.pkgs-errors).buildDepError "vector"))
            ];
