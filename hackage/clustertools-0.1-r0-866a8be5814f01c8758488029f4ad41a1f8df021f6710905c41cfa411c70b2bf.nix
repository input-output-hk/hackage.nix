{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    components = {
      exes = {
        "filter" = {
          depends = [ (hsPkgs.base) (hsPkgs.containers) (hsPkgs.simpleargs) ];
          };
        "clusc" = { depends = [ (hsPkgs.bytestring) ]; };
        "add_single" = {};
        "ace2contigs" = { depends = [ (hsPkgs.bio) ]; };
        "ace2fasta" = {};
        "clusterlibs" = { depends = [ (hsPkgs.regex-compat) ]; };
        };
      };
    }