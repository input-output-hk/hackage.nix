...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.16";
      identifier = { name = "ihaskell"; version = "0.9.0.3"; };
      license = "MIT";
      copyright = "";
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.binpkgdb) (hsPkgs."bin-package-db" or (buildDepError "bin-package-db"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.0") (hsPkgs."ghc-boot" or (buildDepError "ghc-boot"));
        };
      exes = {
        "ihaskell" = {
          depends = [
