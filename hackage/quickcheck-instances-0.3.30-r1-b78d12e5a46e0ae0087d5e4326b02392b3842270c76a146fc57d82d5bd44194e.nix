{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "quickcheck-instances"; version = "0.3.30"; };
    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && ((compiler.version).ge "8.0" && (compiler.version).lt "9.4")) (hsPkgs."data-array-byte" or (errorHandler.buildDepError "data-array-byte"))) ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8.0")) (hsPkgs."semigroups" or (errorHandler.buildDepError "semigroups"))) ++ (pkgs.lib).optionals (!(compiler.isGhc && (compiler.version).ge "7.10")) [
