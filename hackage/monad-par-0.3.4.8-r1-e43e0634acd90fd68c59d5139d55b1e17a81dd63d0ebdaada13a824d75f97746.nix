{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "monad-par"; version = "0.3.4.8"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.chaselev) (hsPkgs."chaselev-deque" or (errorHandler.buildDepError "chaselev-deque"))) ++ (pkgs.lib).optional (flags.newgeneric) (hsPkgs."par-classes" or (errorHandler.buildDepError "par-classes"));
        buildable = true;
        };
      tests = {
        "test-monad-par" = {
