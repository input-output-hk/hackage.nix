    ...
    components = {
      "library" = {
        depends = (([
          ...
          (hsPkgs.nats)
          (hsPkgs.transformers)
          ]) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.0") (hsPkgs.semigroups)) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "7.6.1") ((((pkgs.lib).optional (flags.containers) (hsPkgs.containers) ++ (pkgs.lib).optional (flags.hashable) (hsPkgs.hashable)) ++ (pkgs.lib).optional (flags.hashable && flags.unordered-containers) (hsPkgs.unordered-containers)) ++ (pkgs.lib).optional (flags.vector) (hsPkgs.vector));
        };
      };
    }