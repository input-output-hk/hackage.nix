    ...
    components = {
      "library" = {
        depends = ((([
          ...
          (hsPkgs.syb)
          (hsPkgs.tree-view)
          ] ++ (pkgs.lib).optional (compiler.isGhc && false) (hsPkgs.base)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.10") (hsPkgs.base-orphans)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.8") (hsPkgs.tagged)) ++ (pkgs.lib).optional (flags.th) (hsPkgs.template-haskell);
        };
      tests = {
        "examples" = {
          depends = [
