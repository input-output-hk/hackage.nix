{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { embed_linear = true; };
    package = {
      specVersion = "1.10";
      identifier = { name = "Rasterific"; version = "0.7.2.1"; };
    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (!flags.embed_linear) (hsPkgs."linear" or ((hsPkgs.pkgs-errors).buildDepError "linear"));
        buildable = true;
        };
      };
    }