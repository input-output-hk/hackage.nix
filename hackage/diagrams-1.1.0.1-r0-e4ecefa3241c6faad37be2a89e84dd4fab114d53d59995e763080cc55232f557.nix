{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    flags = { cairo = false; gtk = false; svg = true; ps = false; };
    package = {
      specVersion = "1.10";
      identifier = { name = "diagrams"; version = "1.1.0.1"; };
    ...
    components = {
      "library" = {
        depends = ((([
          ...
          ] ++ (pkgs.lib).optional (flags.cairo) (hsPkgs."diagrams-cairo" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-cairo"))) ++ (pkgs.lib).optional (flags.gtk) (hsPkgs."diagrams-gtk" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-gtk"))) ++ (pkgs.lib).optional (flags.svg) (hsPkgs."diagrams-svg" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-svg"))) ++ (pkgs.lib).optional (flags.ps) (hsPkgs."diagrams-postscript" or ((hsPkgs.pkgs-errors).buildDepError "diagrams-postscript"));
        buildable = true;
        };
      };
    }