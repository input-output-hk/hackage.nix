      ...
      exes = {
        "git-annex" = {
          depends = (((((((([
            ...
            (hsPkgs.http-conduit)
            (hsPkgs.xml-conduit)
            ]) ++ (pkgs.lib).optional (flags.assistant && !system.isWindows && !system.isSolaris) (hsPkgs.stm)) ++ (pkgs.lib).optional (system.isLinux && flags.inotify) (hsPkgs.hinotify)) ++ (pkgs.lib).optional (system.isLinux && flags.dbus) (hsPkgs.dbus)) ++ (pkgs.lib).optionals (flags.webapp && flags.assistant) [
            (hsPkgs.yesod)
            (hsPkgs.yesod-static)
