    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.11") (hsPkgs."transformers" or (errorHandler.buildDepError "transformers"))) ++ (pkgs.lib).optional (!flags.haste && flags.localcache) (hsPkgs."psqueues" or (errorHandler.buildDepError "psqueues"));
        buildable = true;
        };
      };
    }