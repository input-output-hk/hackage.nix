{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "hdevtools"; version = "0.1.2.0"; };
      license = "MIT";
      ...
      exes = {
        "hdevtools" = {
          depends = (([
            ...
            ] ++ (pkgs.lib).optional (compiler.isGhc && false) (hsPkgs."Cabal" or ((hsPkgs.pkgs-errors).buildDepError "Cabal"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.7") (hsPkgs."Cabal" or ((hsPkgs.pkgs-errors).buildDepError "Cabal"))) ++ (pkgs.lib).optionals (compiler.isGhc && (compiler.version).ge "7.9") [
