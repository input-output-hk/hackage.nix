    ...
    components = {
      "library" = {
        depends = ([
          ...
          (hsPkgs.deepseq)
          (hsPkgs.mtl)
          ] ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8.0")) (hsPkgs.semigroups)) ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "7.10")) (hsPkgs.bifunctors);
        };
      tests = {
        "test-alga" = {
          depends = ([
