...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "cabal-rpm"; version = "2.0.1"; };
        ...
        "cabal-rpm" = {
          depends = (([
            ...
            then [
              ...
              (hsPkgs."time" or (buildDepError "time"))
              ])) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.0") (hsPkgs."semigroups" or (buildDepError "semigroups"))) ++ (if compiler.isGhc && (compiler.version).ge "8.0" && !flags.curl
            then [
            ...
            else [ (hsPkgs."curl" or (buildDepError "curl")) ]);
          buildable = true;
          };
        };
      };
