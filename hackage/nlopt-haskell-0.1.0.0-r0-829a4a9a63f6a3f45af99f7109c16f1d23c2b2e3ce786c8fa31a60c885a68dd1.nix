{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "nlopt-haskell"; version = "0.1.0.0"; };
    ...
    components = {
      "library" = {
        ...
        libs = if flags.link-nlopt-cxx
          then [
            (pkgs."nlopt_cxx" or ((hsPkgs.pkgs-errors).sysDepError "nlopt_cxx"))
            ]
          else [ (pkgs."nlopt" or ((hsPkgs.pkgs-errors).sysDepError "nlopt")) ];
        buildable = true;
        };
      tests = {
        "example" = {
