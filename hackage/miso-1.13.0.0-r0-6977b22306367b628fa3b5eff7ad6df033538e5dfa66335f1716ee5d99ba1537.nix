{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "2.2";
      identifier = { name = "miso"; version = "1.13.0.0"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ])) ++ pkgs.lib.optional (flags.template-haskell) (hsPkgs."template-haskell" or (errorHandler.buildDepError "template-haskell"))) ++ pkgs.lib.optionals (flags.aeson) [
