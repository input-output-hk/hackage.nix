    ...
    components = {
      "library" = {
        ...
        build-tools = if flags.mcr
          then [
            (hsPkgs.buildPackages.mcr or (pkgs.buildPackages.mcr or ((hsPkgs.pkgs-errors).buildToolDepError "mcr")))
            ]
          else [
            (hsPkgs.buildPackages.matlab or (pkgs.buildPackages.matlab or ((hsPkgs.pkgs-errors).buildToolDepError "matlab")))
            ];
        buildable = true;
        };
      };
