      ...
      tests = {
        "testsuite" = {
          depends = ([
            ...
            (hsPkgs.vector)
            (hsPkgs.xmlhtml)
            ] ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8.0")) (hsPkgs.semigroups)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.8" && (compiler.isGhc && (compiler.version).lt "7.10")) (hsPkgs.transformers-compat);
          };
        };
      benchmarks = {
        "benchmark" = {
