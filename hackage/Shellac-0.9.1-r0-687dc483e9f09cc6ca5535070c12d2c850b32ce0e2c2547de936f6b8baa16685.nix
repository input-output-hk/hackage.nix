{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.2";
      identifier = { name = "Shellac"; version = "0.9.1"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optionals (!system.isWindows) ((pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "6.8") (hsPkgs."unix" or ((hsPkgs.pkgs-errors).buildDepError "unix")))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "6.8") (hsPkgs."directory" or ((hsPkgs.pkgs-errors).buildDepError "directory"));
        buildable = true;
        };
      };
    }