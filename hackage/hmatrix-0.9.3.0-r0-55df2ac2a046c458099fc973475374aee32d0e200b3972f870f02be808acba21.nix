{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.2";
      identifier = { name = "hmatrix"; version = "0.9.3.0"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.vector) (hsPkgs."vector" or (errorHandler.buildDepError "vector"))) ++ (pkgs.lib).optionals (flags.tests) [
