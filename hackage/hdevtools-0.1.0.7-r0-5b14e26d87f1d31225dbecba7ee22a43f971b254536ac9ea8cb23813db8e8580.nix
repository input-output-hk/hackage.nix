{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "hdevtools"; version = "0.1.0.7"; };
      license = "MIT";
      ...
      exes = {
        "hdevtools" = {
          depends = (([
            ...
            ] ++ (pkgs.lib).optional (compiler.isGhc && ((compiler.version).ge "7.6" && (compiler.version).lt "7.7")) (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.7") (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.9") (hsPkgs."Cabal" or (errorHandler.buildDepError "Cabal"));
          buildable = true;
          };
        };
      };
