{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.6";
      identifier = { name = "ncurses"; version = "0.2.15"; };
      ...
      "library" = {
        ...
        pkgconfig = (pkgs.lib).optionals (flags.use-pkgconfig) (if flags.force-narrow-library
          then [
            ...
            (pkgconfPkgs."panel" or ((hsPkgs.pkgs-errors).pkgConfDepError "panel"))
            ]
          else [
            (pkgconfPkgs."ncursesw" or ((hsPkgs.pkgs-errors).pkgConfDepError "ncursesw"))
            (pkgconfPkgs."panelw" or ((hsPkgs.pkgs-errors).pkgConfDepError "panelw"))
            ]);
        build-tools = [
