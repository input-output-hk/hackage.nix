{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "options"; version = "1.2"; };
      license = "MIT";
      copyright = "";
    ...
    components = {
      "library" = {
        depends = [
          ...
          ] ++ (pkgs.lib).optional (!system.isWindows && (compiler.isGhc && (compiler.version).lt "7.2")) (hsPkgs."bytestring" or (errorHandler.buildDepError "bytestring"));
        buildable = true;
        };
      tests = {
        "options_tests" = {
