    ...
    components = {
      "library" = {
        ...
        libs = if system.isWindows
          then [
            (pkgs."hg3dcegui031" or ((hsPkgs.pkgs-errors).sysDepError "hg3dcegui031"))
            ]
          else [
            (pkgs."hg3dcegui031" or ((hsPkgs.pkgs-errors).sysDepError "hg3dcegui031"))
            (pkgs."CEGUIBase-0.7.7" or ((hsPkgs.pkgs-errors).sysDepError "CEGUIBase-0.7.7"))
            (pkgs."CEGUIOgreRenderer-0.7.7" or ((hsPkgs.pkgs-errors).sysDepError "CEGUIOgreRenderer-0.7.7"))
            ];
        buildable = true;
        };
      };
