{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "Agda"; version = "2.6.2.2"; };
    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ] ++ (pkgs.lib).optional (flags.enable-cluster-counting) (hsPkgs."text-icu" or (errorHandler.buildDepError "text-icu"))) ++ (pkgs.lib).optional (system.isWindows) (hsPkgs."Win32" or (errorHandler.buildDepError "Win32"))) ++ (pkgs.lib).optional (compiler.isGhc && ((compiler.version).ge "8.6" && (compiler.version).lt "8.7")) (hsPkgs."transformers" or (errorHandler.buildDepError "transformers"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4.1" || compiler.isGhc && (compiler.version).gt "8.4.3") (hsPkgs."text" or (errorHandler.buildDepError "text"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.4") (hsPkgs."ghc-compact" or (errorHandler.buildDepError "ghc-compact"));
        build-tools = [
