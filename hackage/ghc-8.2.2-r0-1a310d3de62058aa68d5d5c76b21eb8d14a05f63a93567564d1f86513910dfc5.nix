    ...
    components = {
      "library" = {
        depends = (([
          ...
          (hsPkgs.ghci)
          (hsPkgs.hoopl)
          ] ++ (pkgs.lib).optional (!(system.isLinux && system.isX86_64 && (compiler.isGhc && (compiler.version).eq "8.2.2"))) (hsPkgs.base)) ++ (pkgs.lib).optional (!flags.buildable) (hsPkgs.base)) ++ (if system.isWindows
          then [ (hsPkgs.Win32) ]
          else [
            (hsPkgs.unix)
