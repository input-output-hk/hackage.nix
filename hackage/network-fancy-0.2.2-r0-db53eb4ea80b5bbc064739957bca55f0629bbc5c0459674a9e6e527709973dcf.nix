{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.8";
      identifier = { name = "network-fancy"; version = "0.2.2"; };
    ...
    components = {
      "library" = {
        ...
        libs = if system.isWindows
          ...
          else (pkgs.lib).optional (system.isSolaris) (pkgs."socket" or ((hsPkgs.pkgs-errors).sysDepError "socket")) ++ (pkgs.lib).optional (system.isLinux) (pkgs."pthread" or ((hsPkgs.pkgs-errors).sysDepError "pthread"));
        buildable = true;
        };
      };
    }