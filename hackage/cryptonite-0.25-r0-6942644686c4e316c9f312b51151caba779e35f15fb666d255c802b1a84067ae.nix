{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.18";
      identifier = { name = "cryptonite"; version = "0.25"; };
    ...
    components = {
      "library" = {
        ...
        libs = (pkgs.lib).optional (system.isLinux) (pkgs."pthread" or ((hsPkgs.pkgs-errors).sysDepError "pthread")) ++ (pkgs.lib).optional (system.isWindows) (pkgs."advapi32" or ((hsPkgs.pkgs-errors).sysDepError "advapi32"));
        buildable = true;
        };
      tests = {
        "test-cryptonite" = {
