{ system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.18";
      identifier = { name = "leancheck"; version = "0.6.4"; };
    ...
    components = {
      ...
      tests = {
        "test" = { depends = [ (hsPkgs.base) (hsPkgs.template-haskell) ]; };
        "types" = { depends = [ (hsPkgs.base) (hsPkgs.template-haskell) ]; };
        "tiers" = { depends = [ (hsPkgs.base) (hsPkgs.template-haskell) ]; };
        "operators" = {
          depends = [ (hsPkgs.base) (hsPkgs.template-haskell) ];
          };
        "derive" = { depends = [ (hsPkgs.base) (hsPkgs.template-haskell) ]; };
        "error" = { depends = [ (hsPkgs.base) (hsPkgs.template-haskell) ]; };
        };
      };
    }