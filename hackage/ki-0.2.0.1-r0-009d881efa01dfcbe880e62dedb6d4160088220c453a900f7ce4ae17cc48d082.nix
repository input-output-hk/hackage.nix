{ system
  ...
  , ... }:
  {
    flags = { dejafu-tests = false; };
    package = {
      specVersion = "2.2";
      identifier = { name = "ki"; version = "0.2.0.1"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (!flags.dejafu-tests) (hsPkgs."stm" or (errorHandler.buildDepError "stm"))) ++ (pkgs.lib).optionals (flags.dejafu-tests) [
      ...
      tests = {
        "dejafu-tests" = {
          ...
          buildable = if flags.dejafu-tests then true else false;
          };
        "unit-tests" = {
          depends = [
            (hsPkgs."base" or (errorHandler.buildDepError "base"))
          ...
          buildable = if flags.dejafu-tests then false else true;
          };
        };
      };
    }