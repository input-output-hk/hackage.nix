{ system
  ...
  , ... }:
  {
    flags = { use-pkg-config = false; homebrew-libsodium = false; };
    package = {
      specVersion = "3.0";
      identifier = { name = "libsodium-bindings"; version = "0.0.1.1"; };
    ...
    components = {
      "library" = {
        ...
        libs = (pkgs.lib).optional (!flags.use-pkg-config) (pkgs."sodium" or (errorHandler.sysDepError "sodium"));
        pkgconfig = (pkgs.lib).optional (flags.use-pkg-config) (pkgconfPkgs."libsodium" or (errorHandler.pkgConfDepError "libsodium"));
        buildable = true;
        };
      };
    }