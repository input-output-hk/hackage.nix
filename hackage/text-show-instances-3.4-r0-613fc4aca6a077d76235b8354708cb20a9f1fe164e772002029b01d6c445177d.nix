    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (flags.ghc-boot) (hsPkgs."ghc-boot" or (buildDepError "ghc-boot"))) ++ (if system.isWindows
      ...
      tests = {
        "spec" = {
          depends = (([
            ...
            (hsPkgs."vector" or (buildDepError "vector"))
            (hsPkgs."xhtml" or (buildDepError "xhtml"))
            ] ++ (pkgs.lib).optional (!flags.developer) (hsPkgs."text-show-instances" or (buildDepError "text-show-instances"))) ++ (pkgs.lib).optional (flags.ghc-boot) (hsPkgs."ghc-boot" or (buildDepError "ghc-boot"))) ++ (if system.isWindows
            then [ (hsPkgs."Win32" or (buildDepError "Win32")) ]
            else [
              (hsPkgs."terminfo" or (buildDepError "terminfo"))
              (hsPkgs."unix" or (buildDepError "unix"))
