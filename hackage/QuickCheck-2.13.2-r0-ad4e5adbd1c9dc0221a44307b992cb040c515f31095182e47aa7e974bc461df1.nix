    ...
    components = {
      "library" = {
        depends = (((((([
          ...
          (hsPkgs.random)
          (hsPkgs.containers)
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.2") (hsPkgs.random)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.4") (hsPkgs.containers)) ++ (pkgs.lib).optionals (compiler.isGhc && true) [
          (hsPkgs.transformers)
          (hsPkgs.deepseq)
          ]) ++ (pkgs.lib).optional (compiler.isGhc && true && flags.templatehaskell) (hsPkgs.template-haskell)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.2" && (compiler.isGhc && (compiler.version).lt "7.6")) (hsPkgs.ghc-prim)) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "7.0") (hsPkgs.splitmix)) ++ (pkgs.lib).optionals (compiler.isUhc && true) [
          (hsPkgs.old-time)
          (hsPkgs.old-locale)
          ];
        };
