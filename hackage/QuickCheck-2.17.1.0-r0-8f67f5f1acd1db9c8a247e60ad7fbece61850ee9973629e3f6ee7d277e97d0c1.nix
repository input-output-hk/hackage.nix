{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "QuickCheck"; version = "2.17.1.0"; };
    ...
    components = {
      "library" = {
        ...
        ]) ++ pkgs.lib.optional (!(compiler.isHugs && true)) (hsPkgs."splitmix" or (errorHandler.buildDepError "splitmix"))) ++ pkgs.lib.optionals (compiler.isGhc && true || compiler.isMhs && true) [
          (hsPkgs."transformers" or (errorHandler.buildDepError "transformers"))
          (hsPkgs."deepseq" or (errorHandler.buildDepError "deepseq"))
        ]) ++ pkgs.lib.optional (compiler.isGhc && true && flags.templatehaskell) (hsPkgs."template-haskell" or (errorHandler.buildDepError "template-haskell"))) ++ pkgs.lib.optionals (compiler.isGhc && true) [
          (hsPkgs."random" or (errorHandler.buildDepError "random"))
          (hsPkgs."containers" or (errorHandler.buildDepError "containers"))
      ...
      tests = {
        ...
        "test-quickcheck-instances" = {
          ...
          buildable = if !flags.templatehaskell || compiler.isGhc && compiler.version.le "8.10"
            then false
            else true;
        };
      };
