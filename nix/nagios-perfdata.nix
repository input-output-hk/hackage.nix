{
  "0.2.1" = {
    sha256 = "e34a01dbc9b966f531e74ee4b8fe20ccf2f7b796eb22548e4aae955e0d4b4123";
    revisions = {
      r0 = {
        outPath = "nagios-perfdata-0.2.1-r0-e064700c4b286aa8636f15b0f27aa4bb7f6c2ceacf07b3e5b64af85accaab416.nix";
        revNum = 0;
        sha256 = "e064700c4b286aa8636f15b0f27aa4bb7f6c2ceacf07b3e5b64af85accaab416";
        };
      default = "r0";
      };
    };
  "0.2.2" = {
    sha256 = "42b4be9382c3c6a02c662c33a7443ac30790c4ab7311610f5caabdbe5d213595";
    revisions = {
      r0 = {
        outPath = "nagios-perfdata-0.2.2-r0-f3a4f67e1b0b669dcb26eb00a834b09a4106b739229d32a4fe949670ed14ab08.nix";
        revNum = 0;
        sha256 = "f3a4f67e1b0b669dcb26eb00a834b09a4106b739229d32a4fe949670ed14ab08";
        };
      default = "r0";
      };
    };
  }