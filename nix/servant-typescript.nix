{
  "0.1.0.0" = {
    sha256 = "3569d6d6dd98b2b909788fce32ca0305c27e51fd3e902f17054b9057dca3b52c";
    revisions = {
      r0 = {
        outPath = "servant-typescript-0.1.0.0-r0-2dc94c15b8f532d7676f5b79747ffe7518eb978c97c9ff0111d8589a6041e732.nix";
        revNum = 0;
        sha256 = "2dc94c15b8f532d7676f5b79747ffe7518eb978c97c9ff0111d8589a6041e732";
        };
      default = "r0";
      };
    };
  "0.1.0.1" = {
    sha256 = "4589bbb19261872a368342537cb2f99d646021d320490faea614dffa6769d48f";
    revisions = {
      r0 = {
        outPath = "servant-typescript-0.1.0.1-r0-7a2fd53d72fe84a8ed837acc0ff9416b4d39491000a87f42d8a062a8512430e4.nix";
        revNum = 0;
        sha256 = "7a2fd53d72fe84a8ed837acc0ff9416b4d39491000a87f42d8a062a8512430e4";
        };
      default = "r0";
      };
    };
  "0.1.0.2" = {
    sha256 = "554df288c3dba436a17c4c1cad5b96f73350632a68b756bb6a571e1ff123ce0e";
    revisions = {
      r0 = {
        outPath = "servant-typescript-0.1.0.2-r0-92cfd7f7d96de7a01d3834ef50daec181c54b94b1daa3f5a497d5561b86ea7d3.nix";
        revNum = 0;
        sha256 = "92cfd7f7d96de7a01d3834ef50daec181c54b94b1daa3f5a497d5561b86ea7d3";
        };
      default = "r0";
      };
    };
  }