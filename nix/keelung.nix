{
  "0.9.0.0" = {
    sha256 = "9f484975ec1a7223feac539effb7b577f42308715c81263d3e25235ed8869a04";
    revisions = {
      r0 = {
        nix = import ../hackage/keelung-0.9.0.0-r0-a6a729a54e158274d90b25f5dab8ea054685b521deb832b3e7f1f5ac279bbd3a.nix;
        revNum = 0;
        sha256 = "a6a729a54e158274d90b25f5dab8ea054685b521deb832b3e7f1f5ac279bbd3a";
        };
      default = "r0";
      };
    };
  "0.9.1.0" = {
    sha256 = "ba3668b546426c933c2467a98d331b2d60d353b50682b98cd2b584102b6f6909";
    revisions = {
      r0 = {
        nix = import ../hackage/keelung-0.9.1.0-r0-7d1a53d5163f790b05eae2d338cadf940d2ca15986e2465aa65ef08046d3c19b.nix;
        revNum = 0;
        sha256 = "7d1a53d5163f790b05eae2d338cadf940d2ca15986e2465aa65ef08046d3c19b";
        };
      default = "r0";
      };
    };
  "0.9.2" = {
    sha256 = "bd4a6a62bc18f80c78ed48b1372a07a112b89bac7e5b669d3757ee8a0eb54ec1";
    revisions = {
      r0 = {
        nix = import ../hackage/keelung-0.9.2-r0-5f034ccdd02e1eb54998a5dea0fdc0a45986e4f01f4e19ef87d1190db0e38650.nix;
        revNum = 0;
        sha256 = "5f034ccdd02e1eb54998a5dea0fdc0a45986e4f01f4e19ef87d1190db0e38650";
        };
      default = "r0";
      };
    };
  }