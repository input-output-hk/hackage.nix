{
  "0.1.0.0" = {
    sha256 = "ce86407773f3df8e3f214c9e6303f4914d7a4bdf86badc026c96c22f98ff4d4c";
    revisions = {
      r0 = {
        nix = import ../hackage/yaftee-conduit-bytestring-ft-0.1.0.0-r0-181184f321ea2bb2a79484f8fd90b77594a05bbc4019f3ffd5b2675fa8daadc5.nix;
        revNum = 0;
        sha256 = "181184f321ea2bb2a79484f8fd90b77594a05bbc4019f3ffd5b2675fa8daadc5";
      };
      default = "r0";
    };
  };
  "0.1.0.1" = {
    sha256 = "7456ed825eb3fafd568f3cf24fd4c8804f04b6c8c157bd8bef339d4edb23f4e0";
    revisions = {
      r0 = {
        nix = import ../hackage/yaftee-conduit-bytestring-ft-0.1.0.1-r0-7efd6b94a83e5ec1683595aef500e2c192292c50674fe50d0ffb98638c8f7f5a.nix;
        revNum = 0;
        sha256 = "7efd6b94a83e5ec1683595aef500e2c192292c50674fe50d0ffb98638c8f7f5a";
      };
      default = "r0";
    };
  };
}