{
  "0.1.0.0" = {
    sha256 = "a58167f89fbdbe0f810403c52f77ebd0567763f809ed5eb5109d354e8d08e82f";
    revisions = {
      r0 = {
        nix = import ../hackage/grisette-monad-coroutine-0.1.0.0-r0-a0bccf6b5a5da33981efc7b9d2b41f09452c572ed3d9b42adcea10d19103f1fc.nix;
        revNum = 0;
        sha256 = "a0bccf6b5a5da33981efc7b9d2b41f09452c572ed3d9b42adcea10d19103f1fc";
        };
      default = "r0";
      };
    };
  "0.2.0.0" = {
    sha256 = "416a0e2ac1f997bb47beb08ef911b82bf041669f46fe8b9e9445006d6723fb7e";
    revisions = {
      r0 = {
        nix = import ../hackage/grisette-monad-coroutine-0.2.0.0-r0-c0160cbf181cb6caa945d71d5506d61556bb3e8555394f5e3e4c1f91eb1f0e6b.nix;
        revNum = 0;
        sha256 = "c0160cbf181cb6caa945d71d5506d61556bb3e8555394f5e3e4c1f91eb1f0e6b";
        };
      default = "r0";
      };
    };
  }