{
  "0.4.0" = {
    sha256 = "b8505d0185445afdddb64080c6e041d80432e7113e63999bd04d74d8a9520c82";
    revisions = {
      r0 = {
        nix = import ../hackage/gogol-bigtableadmin-0.4.0-r0-5a4ccdacdf40d51d75bd6f169056df61b4398500363063ce7578bc16cef64128.nix;
        revNum = 0;
        sha256 = "5a4ccdacdf40d51d75bd6f169056df61b4398500363063ce7578bc16cef64128";
      };
      default = "r0";
    };
  };
  "0.5.0" = {
    sha256 = "fd7db779c1d5bfd8784c34ba9f4e5a9cd132d2be5f2f592b64b05e3d9c67b95b";
    revisions = {
      r0 = {
        nix = import ../hackage/gogol-bigtableadmin-0.5.0-r0-57c29b2751849a5d53858139e2a573f489b31f8af67dc26f32328635b2b3c5c5.nix;
        revNum = 0;
        sha256 = "57c29b2751849a5d53858139e2a573f489b31f8af67dc26f32328635b2b3c5c5";
      };
      default = "r0";
    };
  };
}