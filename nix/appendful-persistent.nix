{
  "0.0.0.0" = {
    sha256 = "14de3559669e620f434ca8ad14a1a223948152ddc0a28c349c0cfd3ca76c1040";
    revisions = {
      r0 = {
        nix = import ../hackage/appendful-persistent-0.0.0.0-r0-6d8ef42e60ec6edcacb7b81351168c3271f3add68bc1275160b7793097f05bff.nix;
        revNum = 0;
        sha256 = "6d8ef42e60ec6edcacb7b81351168c3271f3add68bc1275160b7793097f05bff";
        };
      default = "r0";
      };
    };
  "0.1.0.0" = {
    sha256 = "3370c0614ed6007d057085055af389e37b5b9075450c7ee4ff4a7a5ce430842b";
    revisions = {
      r0 = {
        nix = import ../hackage/appendful-persistent-0.1.0.0-r0-92ad1ef421c0c6092c61c0b5d554c7932f92209b3f07b03555fc04f3ecfead85.nix;
        revNum = 0;
        sha256 = "92ad1ef421c0c6092c61c0b5d554c7932f92209b3f07b03555fc04f3ecfead85";
        };
      default = "r0";
      };
    };
  "0.1.0.1" = {
    sha256 = "6e4486b2544cfaccf7fa493c9e50a056b43b8c6e03add0480c1808e6b144870a";
    revisions = {
      r0 = {
        nix = import ../hackage/appendful-persistent-0.1.0.1-r0-cc68a9de2580943395ee86d3c2ec3d480202e66bed6e1734b4a8a79e492d5692.nix;
        revNum = 0;
        sha256 = "cc68a9de2580943395ee86d3c2ec3d480202e66bed6e1734b4a8a79e492d5692";
        };
      default = "r0";
      };
    };
  }