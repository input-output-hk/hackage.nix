{
  "1.0.2" = {
    sha256 = "f47f82aa398ee9690924713869e9706e7037e43818f535b488ec006378faa110";
    revisions = {
      r0 = {
        nix = import ../hackage/xdg-basedir-compliant-1.0.2-r0-b63d6d0d6b7a5f215cabeb0a18807708362f3927755fe39bf0ceb41bcbdfd547.nix;
        revNum = 0;
        sha256 = "b63d6d0d6b7a5f215cabeb0a18807708362f3927755fe39bf0ceb41bcbdfd547";
        };
      default = "r0";
      };
    };
  "1.1.0" = {
    sha256 = "ba30ded1f4479eb82344ddd61019e03f8fd9c996bcf673b52d7d15e52044a396";
    revisions = {
      r0 = {
        nix = import ../hackage/xdg-basedir-compliant-1.1.0-r0-249175e583fc06985b8636b6985703244b1b6b74c5647aac50289b0eab15b524.nix;
        revNum = 0;
        sha256 = "249175e583fc06985b8636b6985703244b1b6b74c5647aac50289b0eab15b524";
        };
      default = "r0";
      };
    };
  "1.2.0" = {
    sha256 = "053236cea04e97e1bfd51e7bc4cd4c5e11b744b08448bb21634a84b8041019eb";
    revisions = {
      r0 = {
        nix = import ../hackage/xdg-basedir-compliant-1.2.0-r0-09a9a24ebc97593f6af480e66b3e4659011b4cfb46251bb0f036431c3c6086c5.nix;
        revNum = 0;
        sha256 = "09a9a24ebc97593f6af480e66b3e4659011b4cfb46251bb0f036431c3c6086c5";
        };
      default = "r0";
      };
    };
  }