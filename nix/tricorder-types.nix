{
  "0.1.0.0" = {
    sha256 = "ef7f9263e3a63178f66eb04c9a7181215513c71fe80f5ad694f8bd07dd49ffa4";
    revisions = {
      r0 = {
        nix = import ../hackage/tricorder-types-0.1.0.0-r0-a722510702851fd58b63b04e8d13004b0d4ad0df40075452b51e65192902ee57.nix;
        revNum = 0;
        sha256 = "a722510702851fd58b63b04e8d13004b0d4ad0df40075452b51e65192902ee57";
      };
      default = "r0";
    };
  };
  "0.2.0.0" = {
    sha256 = "04f8f5566f6113cc116dd694069bb2c302229622800d59cd5fbe4dd5e44bd8b9";
    revisions = {
      r0 = {
        nix = import ../hackage/tricorder-types-0.2.0.0-r0-fb8aeb2519105ea858d6e642583a67aab73dfc3b7f83d5e3784ca3a234e4f774.nix;
        revNum = 0;
        sha256 = "fb8aeb2519105ea858d6e642583a67aab73dfc3b7f83d5e3784ca3a234e4f774";
      };
      default = "r0";
    };
  };
  "0.3.0.0" = {
    sha256 = "affe065c66d25c8f1f4e8f53e93c4038e31e2637809c0fe901406a563779a174";
    revisions = {
      r0 = {
        nix = import ../hackage/tricorder-types-0.3.0.0-r0-04144794290b1694a3e7876d1250b4fa6d24566f50cd4e3dcc999fe2c20a02a4.nix;
        revNum = 0;
        sha256 = "04144794290b1694a3e7876d1250b4fa6d24566f50cd4e3dcc999fe2c20a02a4";
      };
      default = "r0";
    };
  };
  "0.3.1.0" = {
    sha256 = "8a5e8dfbdc4b93dd37ff6a66bc4dad6322dd5e627e96308f78c2a3e3450d685d";
    revisions = {
      r0 = {
        nix = import ../hackage/tricorder-types-0.3.1.0-r0-6db05d0a23ebe2791834efecccffa7e41954472f264aeba89cc44b7d8f565b74.nix;
        revNum = 0;
        sha256 = "6db05d0a23ebe2791834efecccffa7e41954472f264aeba89cc44b7d8f565b74";
      };
      default = "r0";
    };
  };
  "0.4.0.0" = {
    sha256 = "44d1328d79c33764dd045688b0978e3b89be5f7f4bf9cd949af866b22c9eb2b4";
    revisions = {
      r0 = {
        nix = import ../hackage/tricorder-types-0.4.0.0-r0-5c476c971300562a9b239e99b120d238a9523399bd547b6f1bdef12aa748684e.nix;
        revNum = 0;
        sha256 = "5c476c971300562a9b239e99b120d238a9523399bd547b6f1bdef12aa748684e";
      };
      default = "r0";
    };
  };
  "0.4.1.0" = {
    sha256 = "c253529ccb85b908ae67f34854c11549965541077d358ba5084db94c5127eb48";
    revisions = {
      r0 = {
        nix = import ../hackage/tricorder-types-0.4.1.0-r0-492a322c0efb8da790e32cfc888d2573852dfee4ad6a266ec51362ecbcf0a448.nix;
        revNum = 0;
        sha256 = "492a322c0efb8da790e32cfc888d2573852dfee4ad6a266ec51362ecbcf0a448";
      };
      default = "r0";
    };
  };
}