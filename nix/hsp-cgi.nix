{
  "0.4" = {
    sha256 = "607493e905a214f127e7fa7c80343d6acc99f316373f324a2264fd08c04e9e62";
    revisions = {
      r0 = {
        outPath = "hsp-cgi-0.4-r0-835cf9ce3b4ecd68e9d149a2db2b826d3630d319e09da2a12b37a676f0fb25d5.nix";
        revNum = 0;
        sha256 = "835cf9ce3b4ecd68e9d149a2db2b826d3630d319e09da2a12b37a676f0fb25d5";
        };
      default = "r0";
      };
    };
  "0.4.4" = {
    sha256 = "36049bd050c56d27584448934e6575a9453184c1abcc2f120b4cc4fa31f53d54";
    revisions = {
      r0 = {
        outPath = "hsp-cgi-0.4.4-r0-e29ca1e2ac48b070f2d9a220ddf1cbb3a31146caf4e181788d3ea25bc9989c16.nix";
        revNum = 0;
        sha256 = "e29ca1e2ac48b070f2d9a220ddf1cbb3a31146caf4e181788d3ea25bc9989c16";
        };
      default = "r0";
      };
    };
  }