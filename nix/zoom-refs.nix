{
  "0.0.0.0" = {
    sha256 = "3780dd561d0902772ff3ddca00fc5431c14e469c568381f5ab2f13eaf4e3765d";
    revisions = {
      r0 = {
        outPath = "zoom-refs-0.0.0.0-r0-bba22cef2d0dfb806d74e199c19d88d740786696458e58df30df6b457ff1c61d.nix";
        revNum = 0;
        sha256 = "bba22cef2d0dfb806d74e199c19d88d740786696458e58df30df6b457ff1c61d";
        };
      default = "r0";
      };
    };
  "0.0.0.1" = {
    sha256 = "743c0ed5e93bedf4207838274df02f2d3406c871ce51c00572b43d709978b32b";
    revisions = {
      r0 = {
        outPath = "zoom-refs-0.0.0.1-r0-ba9669ace20d4b4d764fa418c45b29c07c002984987020a19457a085ed2e059b.nix";
        revNum = 0;
        sha256 = "ba9669ace20d4b4d764fa418c45b29c07c002984987020a19457a085ed2e059b";
        };
      default = "r0";
      };
    };
  }