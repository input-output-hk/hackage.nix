{
  "0.1" = {
    sha256 = "1b0ee9cdee38adcf5431f259e8ae4c8f29f0bc93c2f3c36d821f0d6e2544d635";
    revisions = {
      r0 = {
        nix = import ../hackage/text-rope-0.1-r0-0654bbff798a5b32d63de75bdfce3ea9a7b9abf1e3325c620878af9d48061e33.nix;
        revNum = 0;
        sha256 = "0654bbff798a5b32d63de75bdfce3ea9a7b9abf1e3325c620878af9d48061e33";
      };
      default = "r0";
    };
  };
  "0.2" = {
    sha256 = "b14422855e9cc375a31c22758454013ac629671331de6ccb77b496509bcae78b";
    revisions = {
      r0 = {
        nix = import ../hackage/text-rope-0.2-r0-53b9b4cef0b278b9c591cd4ca76543acacf64c9d1bfbc06d0d9a88960446d9a7.nix;
        revNum = 0;
        sha256 = "53b9b4cef0b278b9c591cd4ca76543acacf64c9d1bfbc06d0d9a88960446d9a7";
      };
      default = "r0";
    };
  };
  "0.3" = {
    sha256 = "bfafee27b63885b19eb58120cc97771d127aa8ba9f141f03044d1ac476699337";
    revisions = {
      r0 = {
        nix = import ../hackage/text-rope-0.3-r0-49564982ede8efd89f3ccbabe06e2c1d062684f6a45503001e4a038fa3fcd5c4.nix;
        revNum = 0;
        sha256 = "49564982ede8efd89f3ccbabe06e2c1d062684f6a45503001e4a038fa3fcd5c4";
      };
      default = "r0";
    };
  };
}