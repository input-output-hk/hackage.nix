{
  "0.1.0.0" = {
    sha256 = "00aef9d09c739ccf0c82780c34bc7d5c4ee0e0fe6fa6d48837578838e482d3a9";
    revisions = {
      r0 = {
        nix = import ../hackage/crypto-srp-0.1.0.0-r0-9cc401913d6bab77a9b8e51a3128b16e10ed5d2c96be1dcb0d6ab7f8321586bd.nix;
        revNum = 0;
        sha256 = "9cc401913d6bab77a9b8e51a3128b16e10ed5d2c96be1dcb0d6ab7f8321586bd";
      };
      default = "r0";
    };
  };
  "0.1.0.1" = {
    sha256 = "dc388c2e28b6d514989ea8df192b97bf3de846eb1d86cf1c1e81261402bd404d";
    revisions = {
      r0 = {
        nix = import ../hackage/crypto-srp-0.1.0.1-r0-162f37f5a45932f7d41925015529261dff14a63412e7d7727b81738f7c1d9ce3.nix;
        revNum = 0;
        sha256 = "162f37f5a45932f7d41925015529261dff14a63412e7d7727b81738f7c1d9ce3";
      };
      default = "r0";
    };
  };
}