{
  "0.1.0.0" = {
    sha256 = "826de782988443aec91373bcc376391829261baf43af4d2c3e130a0ee5cab164";
    revisions = {
      r0 = {
        nix = import ../hackage/keel-0.1.0.0-r0-12d86cee197f03296c7627abee8b6812591ec3f99802f0a2fc6fe01a827a24a4.nix;
        revNum = 0;
        sha256 = "12d86cee197f03296c7627abee8b6812591ec3f99802f0a2fc6fe01a827a24a4";
      };
      default = "r0";
    };
  };
  "0.1.1.0" = {
    sha256 = "ea2dd84e01b083b9be1b21735a27c5fd955de58f7eb6035edfff66e4a9e411ed";
    revisions = {
      r0 = {
        nix = import ../hackage/keel-0.1.1.0-r0-9dcfb92d12138358ccecf717fc2c4bb6f4e38692ca0b38384d1ab3ee96f9c9b0.nix;
        revNum = 0;
        sha256 = "9dcfb92d12138358ccecf717fc2c4bb6f4e38692ca0b38384d1ab3ee96f9c9b0";
      };
      default = "r0";
    };
  };
}